export default {
	// 验证位数小数
	validateDecimals(rule, val, cb) {
		if (val === '') return;
		const reg = /^\d+(\.\d+)?$/,
			{ min, max } = rule;
		if (!reg.test(reg)) return cb(new Error('格式错误'));
		if (val < min || val > max) return cb(new Error('超出数值范围'));
		cb();
	},
	...
	validateNaN(rule, value, callback) {
		//验证数字
		let regPos = /^[+]{0,1}(\d+)$/;
		if (!regPos.test(value) && value) {
			callback(new Error('格式错误'));
	...
	validateTel(rule, value, callback) {
		...
		let mobile = /^1[3-9]\d{9}$/,
			phone = /^(0\d{2,3}-)?\d{7,8}$/;
		if (!(mobile.test(value) || phone.test(value)) && value) {
			//020-12345678
			callback(new Error('格式错误'));
		} else {
			callback();
