export default {
	...
	freight(sellerPrice, sellerQuantity, freightRate) {
		return (
			this.sellerGoodsValue(sellerPrice, sellerQuantity) * freightRate || ''
		);
	},
	// 保费 （货价+运费)*保费费率
	premium(sellerPrice, sellerQuantity, freightRate, premiumRate) {
		return (
			(this.sellerGoodsValue(sellerPrice, sellerQuantity) +
				this.freight(sellerPrice, sellerQuantity, freightRate)) *
				this.premiumRate || ''
		);
	},
	// 杂费 货价*杂费费率
	extras(sellerPrice, sellerQuantity, extrasRate) {
		return (
			this.sellerGoodsValue(sellerPrice, sellerQuantity) * this.extrasRate || ''
		);
	},
	// 运保杂费额 保费+杂费+运费
		...
		if (entrustOrderType && dealMode) {
			if (entrustOrderType == '1') {
				if (
					...
					return Math.round(sellerGoodsValue * customsExchangeRate) || '';
				}
			}
	...
	baseCurrency(currency = 0, exchangeRate = 0) {
		let num = this.multiplication(currency, exchangeRate);
    typeof num === 'number' && (num = num.toFixed(4));
    return num
	},
	// 原币金额 = 收款金额 * 原币结算汇率
