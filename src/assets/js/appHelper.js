import { instance as service, isProd, theSea } from '@/assets/js/initApi.js';
import qs from 'qs';
...
let api = {
	...
	// 越海对接接口
	_getTheSeaData({ url, method, data, headers, responseType } = {}) {
		return theSea({
			url,
			method,
			headers: headers
	...
	async initSelect(formObject) {
		...
		try {
			const [...response] = await Promise.all(
				requestArr.map(item => {
					if (
						this.selectOption.get(item) &&
						Array.isArray(this.selectOption.get(item).data) &&
						this.selectOption.get(item).data.length
					)
						return utools.cloneObj(this.selectOption.get(item));

					const data = this.handleFn().get(item)
