<template>
  <div class="index-background">
    ...
    <div class="form-layout first-child-layout">
      <!-- 待办事项 -->
      <to-do-list :item.sync="item" :processVisible.sync="processVisible"></to-do-list>
      <!-- 公告 -->
      <div>
        <el-card class="box-card" shadow="never">
          ...
          <ul>
            <template v-for="(item,index) in dataNotifi.notifilist">
              <li :key="index" class="text item form-layout item-space set-height" v-if="index < 4">
      ...
      <!-- 用户信息 -->
      <div>
        <el-card class="box-card form-layout flex-content" shadow="never">
    ...
    <div class="middle-chart">
      <div>
        <div :style="{ height: '380px'}" id="dailySales"></div>
      </div>
      <div>
        <div :style="{ height: '380px'}" id="turnoverYoy"></div>
      </div>
    </div>
    <!-- 底部原图 -->
    ...
    <!-- 弹出公告 -->
    <el-dialog :visible.sync="notifilistVisible" title="系统公告">
      <el-table :data="dataNotifi.notifilist">
        <el-table-column label="公告标题" property="title">
          <template slot-scope="scope">
            <span style="margin-left: 10px">【公告】{{ scope.row.title }}</span>
          </template>
        </el-table-column>
        <el-table-column label="公告时间" property="publicTime">
          <template slot-scope="scope">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-tooltip class="item" content="查看" effect="dark" placement="right">
              <i @click="handleJump(scope.row.itemCode)" class="el-icon-view"></i>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 弹出流程图 -->
    <el-dialog :visible.sync="processVisible" class="viewdialog">
      <flow-chart :process="item" :visible="processVisible" ref="flowchart"></flow-chart>
    </el-dialog>
...
export default {
  ...
  components: {
    FlowChart,
    IndexExchange: _ => import('@/components/page/IndexExchange.vue'),
    ToDoList: _ => import('@/components/page/ToDoList.vue')
  },
  computed: {
  ...
  mounted() {
    ...
    let turnoverYoy = this.$echarts.init(document.getElementById('turnoverYoy'))
