  ...
  <div class="form-table-dialog">
    ...
    >
      ...
      <keep-alive>
        ...
        <components
          ...
          @handleSlotEvent='handleSlotEvent'
    ...
    <div v-else>
      <components
        ...
        @dialogQueryBarClick="handleBtnClick"
        @rowClickData='rowClickData'
        @dblclickTableRow="data => $emit('dblclickTableRow',data)"
        @handleSlotEvent='handleSlotEvent'
        :btnTarget.sync="btnTarget"
        :multipleTable.sync="multipleTable"
        :pageData.sync="pageData"
      ...
      >
        <template v-slot="slotProps">
          <slot :default='slotProps.default'></slot>
        </template>
      </components>
    </div>
  </div>
...
export default {
  ...
  watch: {
    ...
    multipleTable: {
      handler (cur, pre) {
        if (cur === pre) return;
        this.$emit('update:multipleTable', cur)
      },
      deep: true
    },
    clickCurrentRowData: {
      handler (cur, pre) {
        this.$emit('update:activeRow', cur)
      },
      deep: true
    }
  },
  props: {
     ...
     * @property { String } zIndex css的层叠
     * @property { Boolean } isSubTable 当前是否是子表 如果是 才运行双击查看详情弹窗
     * @property { Object } calculate 需要计算属性的字段
     * @property { Object } addSlot slot的集合
     * 
     */
    zIndex: String,
    dialog: {
