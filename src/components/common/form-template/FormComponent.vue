      ...
      <template v-if="form.length">
        ...
        >
          ...
          <template v-if="titleList[i]">
            <div
              class="cut-off"
              v-if="titleList[i]['visible']"
            >{{titleList[i]['text']}}</div>
          </template>
...
export default {
  ...
  computed: {
    ...
    FormList () {
      ...
      if (Array.isArray(this.formConfig)) return [this.formConfig.sort(formSort)];
  ...
  components: {
    // form组件
    stringComponent: () =>
      import("@/components/common/form-template/form-item-template/StringComponent.vue"),
    numberComponent: () =>
      import("@/components/common/form-template/form-item-template/NumberComponent.vue"),
    checkboxComponent: () =>
      import("@/components/common/form-template/form-item-template/CheckboxComponent.vue"),
    dateComponent: () =>
      import("@/components/common/form-template/form-item-template/DateComponent.vue"),
    labelComponent: () =>
      import("@/components/common/form-template/form-item-template/LabelComponent.vue"),
    selectComponent: () =>
      import("@/components/common/form-template/form-item-template/SelectComponent.vue"),
    timeComponent: () =>
      import("@/components/common/form-template/form-item-template/TimeComponent.vue"),
    textareaComponent: () =>
      import("@/components/common/form-template/form-item-template/TextareaComponent.vue"),
    calculateComponent: () =>
      import("@/components/common/form-template/form-item-template/CalculateComponent.vue"),
    moneyComponent: () => import('@/components/common/form-template/form-item-template/MoneyComponent.vue'),
    calcComponent: () => import('@/components/common/form-template/form-item-template/CalcComponent.vue')
  },
  props: {
    /* v-model所绑定的对象 */
    formModel: {
      ...
      handler (cur, pre) {
        ...
        this.$nextTick(() => {
          ...
          this.$emit(
            "handlerFormVerify",
            isArray
              ? { formModel: (this.$refs["formModel"] || [/* 防止undefined */])[0] }
              : this
  ...
  methods: {
    ...
    // 请求下拉框的集合
    getSelectSet () {
      const TYPE = Object.prototype.toString.call(this.formConfig).slice(8, -1);
        ...
        if (Array.isArray(formConfig)) {
          for (let item of formConfig) {
            if ('list' in item && typeof item.list === 'object') {
              ...
              if ('btn' in item) {
                'btn' in item && item.btn && (item.btn = (+status < 2));
              }
            }
          }
          ...
          'btn' in formConfig && formConfig.btn && (formConfig.btn = (+status < 2))
        }
      }
      set(this.formConfig)
    }
