<template>
  <div class="viewprocess">
    <div class="title">
      <!-- <el-button icon="el-icon-close" circle style="margin-left: 20px;"></el-button> -->
      供应链进口全景流程图
    </div>
    <div class="deptcon">
  ...
  methods: {
    ...
    init() {
      ...
      this.graph.on("node:click", ev => {
        if (ev.item.modelCache.bundle) {
          this.$router.push({
            path: "/" + ev.item.modelCache.bundle.menuCode,
            query: {
              key: ev.item.modelCache.bundle.nodeKeyName,
              code: ev.item.modelCache.bundle.nodeKeyValue
            }
          });
        }
      });
        ...
        if (res.status == "200") {
          ...
          this.data.edges.forEach((item, index) => {
            list.forEach((e, i) => {
              if (list.length > i + 1) {
                if (
                  item.source == e.nodeCode &&
                  item.target == list[i + 1].nodeCode
                ) {
          ...
          this.select.vnode = list[list.length - 1].nodeCode; //活动节点
          this.data.edges.forEach((item, index) => {
            //活动线条
            if (item.target == list[list.length - 1].bizCode) {
