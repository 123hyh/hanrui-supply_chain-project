...
export default {
  ...
  data () {
    return {
      ...
      tableDataKeyObj: {  // tabledataKey取值Obj
  ...
  watch: {
    ...
    'ruleForm.exchangeRate': {
      handler (cur, pre) {
        ...
        this.ruleForm.homeCurrencyAmount = (typeof (this.ruleForm.totalAmount) !== 'number' ? 0 : this.ruleForm.totalAmount) * cur;
      },
      deep: true
    },
    'ruleForm.creditLimitRate': {
      handler (cur, pre) {
        if (cur === undefined) return;
        this.ruleForm.creditLimitAmount = (typeof (this.ruleForm.totalAmount) !== 'number' ? 0 : this.ruleForm.totalAmount) * cur;
      },
      deep: true
    }
  },
  computed: {
    ...
    tableConfigBind: _ => ({
      customerName: _ => require('@/domain/tableconfig/business/Client.js').default,
      agreementName: _ => require('@/domain/tableconfig/business/Agreement.js').default,
      creditCompanyName: _ => require('@/domain/tableconfig/basicdata/CompanyInfo.js').default
    }),
    btnClickAsync () {
      return {
        customerName: (data = {}) => api.queryclientSearch({
          ...data,
          clientNo: this.dialogData.ruleForm.entrustOrderNo
        }),
        agreementName: (data = {}) => api.getAssignmentAgreementData({
          ...data,
          agreementNo: this.dialogData.ruleForm.agreementNo,
          customerCode: this.ruleForm.customerCode
        }),
        creditCompanyName: (data = {}) => api.getCreditCompanyNameData({
          ...data,
  ...
  methods: {
    ...
    async handlePageChange (data) {
      try {
        let { data: { list, count } } = await this.getClickTabData()[this.tabActiveName]({ ...data, ...this.setTabRequestData() });
    ...
    async handlerSubPreservation (rowData) {
      ...
      switch (this.currencySelectInputKey) {
        case 'customerName': {
          let { clientNo, clientName, registerCurrencyName, registerCurrency, exportMaxTaxLimit } = rowData;
          this.ruleForm = {
            ...this.ruleForm,
    ...
    async handlerlineOfCredit () {
      try {
        ...
        const data = await Promise.all([
          ...
          set['cashDeposit'](),
          set['guarantee'](),
          set['unsecuredCreditGranting']({ customerCode: this.ruleForm.customerCode, creditType: '1' }),
          set['exportTaxCredit']({ customerCode: this.ruleForm.customerCode, creditType: '2' }),
          set['exportPriceCredit']({ customerCode: this.ruleForm.customerCode, creditType: '3' })
        ]);
        let creditTypeSet = data.slice(3);
        // 求 总信用 额度
        this.ruleForm.totalAmount = data.reduce((cur, { data: { list } }) => {
          return cur += list.reduce((cur, pre) => {
            return cur += +(pre['mortgageValue'] || pre['depositsCurrency'] || pre['ensureAmount'] || pre['creditAmount'] || 0);
          }, 0);
        }, 0);
        ...
        creditTypeSet.forEach(({ data: { list } }, i) => {
          ...
          switch (i) {
            case 0:
              this.ruleForm.creditLimit = !len ? 0 : list.reduce((cur, pre) => cur += +(pre['creditAmount'] || 0), 0) / 10000
              break;
            case 1:
              this.ruleForm.exportMaxTaxLimit = !len ? 0 : list.reduce((cur, pre) => cur += +(pre['creditAmount'] || 0), 0) / 10000;
              break;
            case 2:
              this.ruleForm.exportAmountLimit = !len ? 0 : list.reduce((cur, pre) => cur += +(pre['creditAmount'] || 0), 0) / 10000;
              break;
          }
        }, this);
        // 求 折本位币
        this.ruleForm.homeCurrencyAmount = this.ruleForm.totalAmount * this.ruleForm.exchangeRate / 10000;
        // 求 放贷授信额度金额
        this.ruleForm.creditLimitAmount = this.ruleForm.totalAmount * this.ruleForm.creditLimitRate / 10000;
    ...
    async rousePopover (data) {
      ...
      const { data: { list, count } } = await this.btnClickAsync[this.searchType]((data instanceof Object) && data);
      this.dialogPaging = 0;
      this.popoverList = list, this.dialogPaging = count;
      this.dialogData = { ...this.dialogData, ...this.setDialogTitleAndQuery[this.searchType] };
    ...
    async handlerTabClick ({ name, ...obj } = tab = {}) {
      ...
      try {
        let { data: { list, count } } = await this.getClickTabData()[name](this.setTabRequestData());
        this.tabTableData = list;
        this.pagingCount = count;
      } catch (error) {
        console.log(error)
    ...
    getClickTabData () {
      ...
      return {
        ...
        'cashDeposit': _ => api.getCashDepositManagement({ customerCode: this.ruleForm.customerCode }),
        'guarantee': _ => api.getEnsureManageList({ securedUnitCode: this.ruleForm.customerCode }),
    ...
    async initData () {
      ...
      if (this.jumpType == "update") {
        ...
        let { data: { list, count } } = await this.getClickTabData()[this.tabActiveName]();
        this.tabTableData = list;
      } else {
        // 修改时初始化操作
        // 新增时请求获取编码
    ...
    async handlerTabDelete ({ itemCode } = data) {
      if (!itemCode) return;
      const { status } = await this.tabDeleteData()[this.tabActiveName](itemCode)
      status == 200 ? this.$message({
        message: '删除成功',
        type: 'success',
        center: true
      })
