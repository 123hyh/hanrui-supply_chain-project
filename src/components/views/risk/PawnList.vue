...
export default {
  ...
  watch: {
    ...
    'formDialog.ruleForm.originalPrice' (cur, pre) {
      if (!cur) return;
      this.formDialog.ruleForm.originalPriceCurrency = (cur || 0) * (this.formDialog.ruleForm.exchangeRate || 0)
    }
  },
  created () {
    this.initTableList();
  ...
  methods: {
    ...
    async submitStatus () {
      let sCode = 0;
      try {
        const { status } = await this.utools.submitBill({ param: this.formDialog.ruleForm, ctx: this })
        sCode = status;
    ...
    async saveForm () {
      ...
      try {
        ...
        s = status;
        this.formDialog.ruleForm = { ...this.formDialog.ruleForm, status: data.status || '1' }
        this.initTableList();
      } catch (error) {
        console.log(error);
      }
    ...
    async handlerFormConfigClickSearch (params) {
      ...
      if (params === "pawnCompanyName" && !this.formDialog.ruleForm.pawnCompanyType) {
        // 必须选择后才可以选择类型才可以选择 抵(质)押物单位
        utools.alertStatusMessage.call(this, { msg: { warning: '请先选择抵(质)押物单位类型' } });
        return;
      }
      if (dataType === 'String' && params !== "search") {
        ...
        if (params == "pawnCompanyName") {
          this.tableDialog.itemName = itemName[this.formDialog.ruleForm.pawnCompanyType]()
        }
      ...
      try {
        const {
          ...
          : this.tableDialogRequest[this.tableDialogTarget])({
            ...
            ...this.tableDialog.ruleForm,
            status: WHITE_LIST.includes(this.tableDialogTarget) ? '2' : ''
          });
  ...
  computed: {
    ...
    queryBarRequest () {
      return {
        ...
        change: method => api.changePawnData(this.formDialog.ruleForm, method)
      };
    },
    tableDialogRequest () {
      return {
    ...
    tableDialogConfig: () => ({
      ...
      pawnCompanyName: {
        ...
        "2": _ =>
          require("@/domain/tableconfig/basicdata/SupplierBase").default,
        "3": _ =>
          require("@/domain/tableconfig/basicdata/ThirdGroup.js").default,
        "4": _ => require("@/domain/tableconfig/business/Client").default
      },
      pawnUnitName: _ =>
