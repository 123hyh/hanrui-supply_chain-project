...
// 注册表
import tableConfig from "@/domain/tableconfig/basicdata/TrainNumber.js";
import formConfig from "@/domain/formconfig/basicdata/TrainNumber.js";
...
export default {
  ...
  methods: {
    async initTableList (params = {}) {
      try {
        const {
          data: { list, count }
        } = await api.getTrainNumberList({ ...this.queryBar.data, ...params });
        ...
        case "add":
          {
            try {
              ...
              this.formDialog.ruleForm.trainNumberCode = data;
        ...
        case "update":
          {
            ...
            if (!this.tableClickRow.trainNumberCode) {
  ...
  computed: {
    ...
    queryBarRequest () {
      return {
        delete: _ => api.deleteTrainNumberData(this.tableClickRow.trainNumberCode),
        add: _ => api.getTrainNumberCode(),
        change: method => api.changeTrainNumberData(this.formDialog.ruleForm, method)
      };
    },
    tableDialogRequest () {
      return {
