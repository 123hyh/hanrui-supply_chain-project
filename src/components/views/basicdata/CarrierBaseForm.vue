<template>
  <div class="Box CarrierBaseForm">
    ...
    >
      ...
      ></form-component>
      <!-- 子页签 -->
      <el-tabs type="card" v-show="tabActiveName === 'finance'">
        <el-tab-pane label="银行信息" name></el-tab-pane>
      </el-tabs>

      <!-- 表格按钮 -->
      <div v-show="isShowTable">
        <div class="mb-10" v-show="(+(form.data.base.status) || 0) <= 1">
          <el-button
            :key="item.key"
            :type="item.type"
...
export default {
  ...
  data: () => ({
    ...
    form: {
      ...
      formConfig: {
        base: utools.cloneObj(carrierBase),
        finance: utools.cloneObj(carrierFinance),
        purchase: utools.cloneObj(carrierPurchase),
        delivery: [],
        credit: utools.cloneObj(carrierCredit),
        car: [],
        driver: []
      },
      validate: eval,
  ...
  computed: {
    ...
    isShowTable() {
      return !['base', 'credit', 'purchase', 'purchase'].includes(
        this.tabActiveName
      )
    },
      ...
      return {
        search: async page => {
          try {
            ...
            let searchApi = this.table.itemTabObj.tabApi[
              flag ? 'searchApi' : 'searchOneApi'
            ]
            let params = flag
              ? {
                  ...page,
                  carrierCode: this.form.data.base.carrierCode
                }
  ...
  watch: {
    'tableDialog.isShowPopover'(val) {
      if (!val) {
        this.tableDialog.count = 0
    ...
    'form.data.base.province': {
      handler(cur, pre) {
        ...
        pre &&
          (delete this.form.data.base.city, delete this.form.data.base.district)
  ...
  methods: {
        ...
        this.form.validate(async valid => {
          if (valid) {
            utools.saveReceiptsTips.call(this, async () => {
              await port[this.tabActiveName].call(api, {
                data: {
                  ...this.form.data[this.tabActiveName],
                  carrierCode: this.form.data.base.carrierCode
                },
                method: (() => {
    ...
    rousePopover(params, obj, queryData) {
      ...
      queryData =
        params == 'goodsName'
          ? {
              ...queryData,
              entrustOrderNos: this.formDialog.ruleForm.orderNos
            }
          : queryData
      // 弹窗条件
      params
    ...
    setTableDialogConfig() {
      let {companyName,carrierbase} = {
        ...
        carrierbase: [
          {
            label: '名称',
            moduleBind: 'carrierName',
            isInput: true
          }
        ]
      }
      this.tableDialog.queryBarConfig = new Map([
        ['financeName', companyName],
        ['purchaseName',companyName],
        ['purchaseCompanyName', carrierbase],
        ['deliveryCompanyName', carrierbase]
      ]).get(this.tableDialog.target)
    },
    handleVerify() {},
    // 黑名单
    ...
    handleTabBtnClick(target) {
      ...
      if (target === 'add') {
        ...
        utools.titleCallBack.call(this, this.table.clickRow, () => {
    ...
    rouserFormDialog() {
      let formConfig = {
        finance: utools.cloneObj(CarrierBank),
        delivery: utools.cloneObj(carrierDelivery),
        car: utools.cloneObj(carrierCar),
        driver: utools.cloneObj(carrierDriver)
      }[this.tabActiveName]
      let dialogTitle = {
        finance: '银行信息',
        delivery: '货仓提货地址',
    ...
    handlerSubPreservation(data) {
      let obj = {
        purchaseCompanyName: {
          purchaseCompany: data.carrierCode,
          purchaseCompanyName: data.carrierName
        },
        deliveryCompanyName: {
          deliveryCompanyCode: data.carrierCode,
          deliveryCompanyName: data.carrierName
        },
        financeName: {
        ...
        orderNos: () => ({
          orderNos: data.map(item => item.entrustOrderNo)
        }),
        goodsName: {
          ...data,
    ...
    handleTableDialogClick() {
      this.rousePopover(this.tableDialog.target, {}, this.tableDialog.ruleForm)
    },
    handlePageChange(data = {}) {
      this.rousePopover(this.tableDialog.target, null, {
        ...data,
        ...this.tableDialog.ruleForm
      })
    },
    handleTabPageChange() {},
    handlerPopFormVerify({ formModel: { validate, resetFields } } = {}) {
    ...
    handleTabsClick: (() => {
      ...
      return async function({ name } = {}) {
        try {
          ...
          this.handlerTablePageData()
          if (
            Object.keys(port).includes(name) &&
            utools.isNull(this.form.data[active])
          ) {
            const { data } = await port[active].call(
              api,
              this.form.data.base.carrierCode
            )
            for (let key in data) {
              if (data.hasOwnProperty(key))
                this.$set(this.form.data[active], key, data[key])
            }
          }
        } catch (error) {
    ...
    async handlerTablePageData(data = {}) {
      ...
      if (Object.keys(port).includes(this.tabActiveName)) {
        const {
          data: { list = [], count = 0 }
        } = await port[active].call(api, {
          // 银行信息的code不同
          [active !== 'finance' ? 'carrierCode' : 'billNo']: this.form.data.base
            .carrierCode,
          ...data
        })
    ...
    async beforeSubmitCb() {
      try {
        const code = this.form.data.base.carrierCode
        const [
          { data: base },
          { data: finance },
          {
          ...
          api.searchOnecarrierbaseData(code),
          api.searchOnecarrierfinanceData(code),
