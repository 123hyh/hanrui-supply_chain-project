...
// 注册表
import tableConfig from "@/domain/tableconfig/basicdata/SupervisionMode.js";
import formConfig from "@/domain/formconfig/basicdata/SupervisionMode.js";
...
export default {
  ...
  methods: {
    async initTableList (params = {}) {
      try {
        const {
          data: { list, count }
        } = await api.getSupervisionModeList({ ...this.queryBar.data, ...params });
        ...
        case "add":
          {
            try {
              ...
              this.formDialog.ruleForm.supervisionModeCode = data;
        ...
        case "update":
          {
            ...
            if (!this.tableClickRow.supervisionModeCode) {
  ...
  computed: {
    ...
    queryBarFormConfig () {
      return [
        { label: "单据编码", moduleBind: "supervisionModeCode", isInput: true },
    ...
    queryBarRequest () {
      return {
        delete: _ => api.deleteSupervisionModeData(this.tableClickRow.supervisionModeCode),
        add: _ => api.getSupervisionModeCode(),
        change: method => api.changeSupervisionModeData(this.formDialog.ruleForm, method)
      };
    },
    tableDialogRequest () {
      return {
