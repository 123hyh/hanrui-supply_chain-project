...
export default {
  ...
  computed: {
    ...
    clickTypeAsync () {
      return {
        ...
        'add': () => (this.formDialog.isShowPopover = true, this.loadCodeNo(), this.formDialog.ruleForm = { ...new ruleForm(), batchRuleCode: this.formDialog.ruleForm.batchRuleCode }),
        ...
        'delete': () => {
          this.utools.titleCallBack.bind(this)(this.formDialog.clickRow, () => {
            this.utools.deleteMessage.bind(this)(async () => {
              try {
                const { status, data: { list, count } } = await api.deletebatchruleData(this.formDialog.clickRow.batchRuleCode);
  ...
  methods: {
    ...
    async saveForm () {
      if (this.isVerify()) {
        try {
          let apiType = this.formDialog.btnKey === 'add' ? 'addbatchruleData' : 'editbatchruleData';
          const {
            data,
            status
    ...
    async loadCodeNo () { //加载编号
      ...
      try {
        ...
        this.formDialog.ruleForm.batchRuleCode = data;
      } catch (e) { console.log(e) }
    },
  },
  created () {
