...
import tableconfig from '@/domain/tableconfig/basicdata/VoucherSummary'
import ruleForm from '@/domain/entities/basicdata/VoucherSummary'
import formConfig from '@/domain/formconfig/basicdata/VoucherSummary'
...
export default {
  ...
  computed: {
    ...
    clickTypeAsync () {
      return {
        ...
        'add': () => (this.formDialog.isShowPopover = true, this.loadCodeNo(), this.formDialog.ruleForm = { ...new ruleForm(), voucherSummaryCode: this.formDialog.ruleForm.voucherSummaryCode }),
        ...
        'delete': () => {
          this.utools.titleCallBack.bind(this)(this.formDialog.clickRow, () => {
            this.utools.deleteMessage.bind(this)(async () => {
              try {
                const { status, data: { list, count } } = await api.deletevouchersummaryData(this.formDialog.clickRow.voucherSummaryCode);
  ...
  methods: {
    ...
    async saveForm () {
      if (this.isVerify()) {
        try {
          let apiType = this.formDialog.btnKey === 'add' ? 'addvouchersummaryData' : 'editvouchersummaryData';
          const {
            data,
            status
    ...
    async loadCodeNo () { //加载编号
      ...
      try {
        ...
        this.formDialog.ruleForm.voucherSummaryCode = data;
      } catch (e) { console.log(e) }
    },
  },
  created () {
