...
import tableconfig from '@/domain/tableconfig/basicdata/DomesticDestination'
import ruleForm from '@/domain/entities/basicdata/DomesticDestination'
import formConfig from '@/domain/formconfig/basicdata/DomesticDestination'
...
export default {
  ...
  computed: {
    ...
    queryBarFormConfig () {
      return [
        { label: "编码", moduleBind: "domesticDestinationCode", isInput: true },
    ...
    clickTypeAsync () {
      return {
        'search': async (page) => {
          try {
            const { data: { list, count } } = await api.querydomesticdestinationSearch({ ...this.ruleForm, ...page });
            this.tableData = list,
        ...
        'add': () => (
          ...
          this.formDialog.ruleForm = {
            ...new ruleForm(),
            domesticDestinationCode: this.formDialog.ruleForm.domesticDestinationCode
          }
        ),
        ...
        'update': () => {
          utools.titleCallBack.call(
            ...
            async () => {
              try {
                const { data } = await api.searchOnedomesticdestinationData(this.formDialog.clickRow.domesticDestinationCode);
        ...
        'delete': () => {
          utools.titleCallBack.call(
            ...
            utools.removeReceiptsTips.bind(
              this,
              async () => {
                await api.deletedomesticdestinationData(this.formDialog.clickRow.domesticDestinationCode);
  ...
  methods: {
    ...
    async submitStatus () {
      utools.sbumitReceiptsTips.call(
        this,
        async () => {
          let { domesticDestinationCode } = this.formDialog.ruleForm;
          await api.updateBillStatus({ type: 'submit', code: domesticDestinationCode });
      ...
      this.formDialog.validate(async valid => {
        if (valid) {
          utools.saveReceiptsTips.call(
            this,
            async () => {
              let apiType = this.formDialog.btnKey === 'add' ? 'adddomesticdestinationData' : 'editdomesticdestinationData';
              const { data } = await api[apiType](this.formDialog.ruleForm);
    ...
    async loadCodeNo () {
      ...
      try {
        const {
          data
        } = await api.getdomesticdestinationCode();
        this.$set(this.formDialog.ruleForm, 'domesticDestinationCode', data)
      } catch (e) {
