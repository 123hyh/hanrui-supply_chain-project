...
import tableconfig from '@/domain/tableconfig/basicdata/TransactionType'
import ruleForm from '@/domain/entities/basicdata/TransactionType'
import formConfig from '@/domain/formconfig/basicdata/TransactionType'
...
export default {
  ...
  computed: {
    ...
    queryBarFormConfig () {
      return [
        { label: "编码", moduleBind: "transactionCode", isInput: true },
    ...
    clickTypeAsync () {
      return {
        'search': async (page) => {
          try {
            const { data: { list, count } } = await api.querytransactiontypeSearch({ ...this.ruleForm, ...page });
            this.tableData = list,
        ...
        'add': () => (
          ...
          this.formDialog.ruleForm = {
            ...new ruleForm(),
            transactionCode: this.formDialog.ruleForm.transactionCode
          }),
        ...
        'update': () => {
          this.utools.titleCallBack.call(
            ...
            async () => {
              try {
                const { data } = await api.searchOnetransactiontypeData(this.formDialog.clickRow.transactionCode);
        ...
        'delete': () => {
          this.utools.titleCallBack.call(
            ...
            utools.removeReceiptsTips.bind(
              this,
              async () => {
                await api.deletetransactiontypeData(this.formDialog.clickRow.transactionCode);
  ...
  methods: {
      ...
      this.formDialog.validate(async valid => {
        if (valid) {
          utools.saveReceiptsTips.call(
            this,
            async () => {
              const { data } = await api.changeTransactiontypeData({
    ...
    async submitStatus () {
      utools.sbumitReceiptsTips.call(
        this,
        async () => {
          let { transactionCode } = this.formDialog.ruleForm;
          await api.updateBillStatus({ type: 'submit', code: transactionCode });
    ...
    async loadCodeNo () {
      ...
      try {
        const {
          data
        } = await api.gettransactiontypeCode()
        this.$set(this.formDialog.ruleForm, 'transactionCode', data)
      } catch (e) {
