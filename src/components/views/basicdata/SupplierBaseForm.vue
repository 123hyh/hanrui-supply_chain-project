      ...
      <section>
        ...
        <div v-show="table.tableDataKey.length > 0 ">
          <tab-table
            ...
            :tabName="(form.data.status || 0) <= 1 ? subActiveName : ''"
          ...
          <pagination-component :count="table.count" @handlePageChange="handleSubTablePageChange"></pagination-component>
        </div>
      </section>
    </section>
    <!-- 弹窗 -->
...
export default {
  ...
  watch: {
    ...
    'form.data.province': {
      handler(cur, pre) {
        ...
        pre && (delete this.form.data.city, delete this.form.data.district)
  ...
  methods: {
    ...
    vet(f) {
      let flog = true
      if (f < 3) {
        this.$refs['mianform'].$refs['formModel'].validate(valid => {
          if (!valid) {
            flog = false
          }
        })
    ...
    async beforeSubmitCb() {
      /* let flog = true
            for (let f = 0; f < this.initTabList.length; f++) {
                this.activeName = this.initTabList[f]['name']
                await this.$nextTick(() => {
                    flog = this.vet(f)
                })
                if (!flog) {
            ...
            return flog */
      try {
        const code = this.form.data.supplierCode
        const data = await Promise.all([
          api.searchOnesupplierbaseData(code),
          api.searchOnesupplierfinanceData(code),
          api.searchOnesupplierpurchaseData(code),
          api.getALLBankDataAll({ billNo: code }),
          api.querysuppliercontactSearch({ supplierCode: code })
        ])
        if (
          data.every(({ data = {} }) => {
  ...
  computed: {
    ...
    initFormConfig: _ => ({
      base: _ =>
        utools.cloneObj(
          require('@/domain/formconfig/basicdata/SupplierBase.js').default
        ),
      finance: _ =>
        utools.cloneObj(
          require('@/domain/formconfig/basicdata/SupplierFinance.js').default
        ),
      purchase: _ =>
        utools.cloneObj(
          require('@/domain/formconfig/basicdata/SupplierPurchase.js').default
        ),
      delivery: _ => [],
      ...
      credit: _ =>
        utools.cloneObj(
          require('@/domain/formconfig/basicdata/SupplierCredit.js').default
        ),
      // 弹窗表单
      bank: _ =>
        utools.cloneObj(
          require('@/domain/formconfig/basicdata/SupplierBank.js').default
        ),
      contact: _ =>
        utools.cloneObj(
          require('@/domain/formconfig/basicdata/SupplierContact.js').default
        )
    }),
    initTableConfig: _ => ({
      ...
      bank: _ =>
        require('@/domain/tableconfig/basicdata/SupplierBank.js').default,
      contact: _ =>
        require('@/domain/tableconfig/basicdata/SupplierContact.js').default,
      address: _ =>
        require('@/domain/tableconfig/basicdata/SupplierDelivery.js').default,
    ...
    dialogAsync() {
      return {
        parentSupplierName: (data = {}) => api.querysupplierbaseSearch(data),
        purchaseCompanyName: (data = {}) => api.querysupplierbaseSearch(data),
        ...
        receivableCustName: (data = {}) => api.querysupplierbaseSearch(data),
        receiptCustName: (data = {}) => api.querysupplierbaseSearch(data),
        deliverCustName: (data = {}) => api.querysupplierbaseSearch(data),
        deliveryNo: (data = {}) => api.querysupplierdeliverySearch(data),
    ...
    preserveAsync() {
      return {
        base: () => api.changesupplierbase(this.form.data, this.setMethod()),
    ...
    switchTabAsync() {
      return {
        base: _ => api.searchOnesupplierbaseData(this.form.data.supplierCode),
        finance: _ =>
          api.searchOnesupplierfinanceData(this.form.data.supplierCode),
        purchase: _ =>
          api.searchOnesupplierpurchaseData(this.form.data.supplierCode),
        delivery: _ =>
          api.searchOnesupplierdeliveryData(this.form.data.supplierCode),
        credit: _ =>
