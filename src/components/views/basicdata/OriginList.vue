...
// 注册表
import tableConfig from "@/domain/tableconfig/basicdata/Origin.js";
import formConfig from "@/domain/formconfig/basicdata/Origin.js";
...
export default {
  ...
  methods: {
    async initTableList (params = {}) {
      try {
        const {
          data: { list, count }
        } = await api.getOriginList({ ...this.queryBar.data, ...params });
        ...
        case "add":
          {
            try {
              ...
              this.formDialog.ruleForm.originCode = data;
        ...
        case "update":
          {
            ...
            if (!this.tableClickRow.originCode) {
  ...
  computed: {
    ...
    queryBarRequest () {
      return {
        delete: _ => api.deleteOriginData(this.tableClickRow.originCode),
        add: _ => api.getOriginCode(),
        change: method => api.editOriginData(this.formDialog.ruleForm, method)
      };
    },
    tableDialogRequest () {
      return {
