...
import bankTable from '@/domain/tableconfig/basicdata/CompanyInfoBankTrial.js'
...
export default {
    ...
    'form.data.base.province': {
      handler(cur, pre) {
        ...
        utools.getLocation({
          location: 'city',
          code: cur,
          form: { formConfig: this.form.formConfig.base, fields: 'city' }
        })
    ...
    'form.data.base.city': {
      handler(cur, pre) {
        ...
        utools.getLocation({
          location: 'district',
          code: cur,
          form: { formConfig: this.form.formConfig.base, fields: 'district' }
        })
        pre && (form.district = '')
      },
      deep: true
  ...
  methods: {
    ...
    handlerFormSearchClick(target) {
      ...
      Promise.all([this.setTableDialog(target), this.getTableDialogPageData()])
    },
    ...
    // 删除银行信息
    removeBankData() {
      const { itemCode, status } = this.table.activeRow
      if (status === '4') return this.$message.warning('无法删除已审核的银行信息！')
      if (this.isTableActive()) {
      ...
      try {
        if ('itemCode' in data) {
          ...
          await api.changeAllBankData({
            data: { ...data, billNo: this.form.data.base.companyCode },
    ...
    vet(f) {
      ...
      if (f == '0') {
        ...
        this.$refs['mianform'].$refs['formModel_1'][0].validate(valid => {
          if (!valid) {
            flog = false
          }
        })
