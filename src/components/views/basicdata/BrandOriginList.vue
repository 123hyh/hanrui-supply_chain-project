...
// 注册表
import tableConfig from "@/domain/tableconfig/basicdata/BrandOrigin.js";
import formConfig from "@/domain/formconfig/basicdata/BrandOrigin.js";
...
export default {
  ...
  methods: {
    async initTableList (params = {}) {
      try {
        const {
          data: { list, count }
        } = await api.getBrandOriginList({ ...this.queryBar.data, ...params });
        ...
        case "add":
          {
            try {
              ...
              this.formDialog.ruleForm.brandOriginCode = data;
        ...
        case "update":
          {
            ...
            if (!this.tableClickRow.brandOriginCode) {
    ...
    setTableDialog (key = '') {
      let config = {
        clientName: {
          title: '委托方',
          queryBarObj: [{ label: "编码", moduleBind: "clientNo", isInput: true }]
        },
        supplierName: {
          title: '供应商',
        ...
        originName: {
          title: '产地',
          queryBarObj: [{ label: "编码", moduleBind: "originCode", isInput: true }]
        },
        '': {}
      }
      return config[key]
  ...
  computed: {
    ...
    queryBarRequest () {
      return {
        delete: _ => api.deleteBrandOriginData(this.tableClickRow.brandOriginCode),
        add: _ => api.getBrandOriginCode(),
        change: method => api.changeBrandOriginData(this.formDialog.ruleForm, method)
      };
    },
    tableDialogRequest () {
      return {
