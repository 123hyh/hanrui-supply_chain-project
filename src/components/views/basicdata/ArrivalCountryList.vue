...
// 注册表
import tableConfig from "@/domain/tableconfig/basicdata/ArrivalCountry.js";
import formConfig from "@/domain/formconfig/basicdata/ArrivalCountry.js";
...
export default {
  ...
  methods: {
    async initTableList (params = {}) {
      try {
        const {
          data: { list, count }
        } = await api.getArrivalCountryList({ ...this.queryBar.data, ...params });
        ...
        case "add":
          {
            try {
              ...
              this.formDialog.ruleForm.arrivalCountryCode = data;
        ...
        case "update":
          {
            ...
            if (!this.tableClickRow.arrivalCountryCode) {
  ...
  computed: {
    ...
    queryBarFormConfig () {
      return [
        { label: "单据编码", moduleBind: "arrivalCountryCode", isInput: true },
    ...
    queryBarRequest () {
      return {
        delete: _ => api.deleteArrivalCountryData(this.tableClickRow.arrivalCountryCode),
        add: _ => api.getArrivalCountryCode(),
        change: method => api.changeArrivalCountryData(this.formDialog.ruleForm, method)
      };
    },
    tableDialogRequest () {
      return {
