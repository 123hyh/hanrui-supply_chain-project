...
// 注册表
import tableConfig from "@/domain/tableconfig/basicdata/Brand.js";
import formConfig from "@/domain/formconfig/basicdata/Brand.js";
...
export default {
  ...
  methods: {
    async initTableList (params = {}) {
      try {
        const {
          data: { list, count }
        } = await api.getBrandList({ ...this.queryBar.data, ...params });
        ...
        case "add":
          {
            try {
              ...
              this.formDialog.ruleForm.brandCode = data;
        ...
        case "update":
          {
            ...
            if (!this.tableClickRow.brandCode) {
  ...
  computed: {
    ...
    queryBarFormConfig () {
      return [
        { label: "编码", moduleBind: "brandCode", isInput: true },
    ...
    queryBarRequest () {
      return {
        delete: _ => api.deleteBrandData(this.tableClickRow.brandCode),
        add: _ => api.getBrandCode(),
        change: method => api.changeBrandData(this.formDialog.ruleForm, method)
      };
    },
    tableDialogRequest () {
      return {
