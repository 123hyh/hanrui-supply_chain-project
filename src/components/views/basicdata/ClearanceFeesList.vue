...
// 注册表
import tableConfig from "@/domain/tableconfig/basicdata/ClearanceFees.js";
import formConfig from "@/domain/formconfig/basicdata/ClearanceFees.js";
...
export default {
  ...
  methods: {
    async initTableList (params = {}) {
      try {
        const {
          data: { list, count }
        } = await api.getClearanceFeesList({ ...this.queryBar.data, ...params });
        ...
        case "add":
          {
            try {
              ...
              this.formDialog.ruleForm.clearanceFeesCode = data;
        ...
        case "update":
          {
            ...
            if (!this.tableClickRow.clearanceFeesCode) {
  ...
  computed: {
    ...
    queryBarFormConfig () {
      return [
        { label: "单据编码", moduleBind: "clearanceFeesCode", isInput: true },
    ...
    queryBarRequest () {
      return {
        delete: _ => api.deleteClearanceFeesData(this.tableClickRow.clearanceFeesCode),
        add: _ => api.getClearanceFeesCode(),
        change: method => api.changeClearanceFeesData(this.formDialog.ruleForm, method)
      };
    },
    tableDialogRequest () {
      return {
