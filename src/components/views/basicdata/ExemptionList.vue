...
// 注册表
import tableConfig from "@/domain/tableconfig/basicdata/Exemption.js";
import formConfig from "@/domain/formconfig/basicdata/Exemption.js";
...
export default {
  ...
  methods: {
    async initTableList (params = {}) {
      try {
        const {
          data: { list, count }
        } = await api.getExemptionList({ ...this.queryBar.data, ...params });
        ...
        case "add":
          {
            try {
              ...
              this.formDialog.ruleForm.exemptionCode = data;
        ...
        case "update":
          {
            ...
            if (!this.tableClickRow.exemptionCode) {
  ...
  computed: {
    ...
    queryBarFormConfig () {
      return [
        { label: "单据编码", moduleBind: "exemptionCode", isInput: true },
    ...
    queryBarRequest () {
      return {
        delete: _ => api.deleteExemptionData(this.tableClickRow.exemptionCode),
        add: _ => api.getExemptionCode(),
        change: method => api.changeExemptionData(this.formDialog.ruleForm, method)
      };
    },
    tableDialogRequest () {
      return {
