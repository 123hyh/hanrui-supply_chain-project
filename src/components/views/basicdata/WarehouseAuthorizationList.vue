...
import tableconfig from '@/domain/tableconfig/basicdata/WarehouseAuthorization'
import ruleForm from '@/domain/entities/basicdata/WarehouseAuthorization'
import formConfig from '@/domain/formconfig/basicdata/WarehouseAuthorization'
...
export default {
  ...
  computed: {
    ...
    queryBarFormConfig () {
      return [
        { label: "编码", moduleBind: "authWarehouseCode", isInput: true },
    ...
    clickTypeAsync () {
      return {
        'search': async (page) => {
          try {
            const { data: { list, count } } = await api.querywarehouseauthorizationSearch({ ...this.ruleForm, ...page });
            this.tableData = utools.toggleDataBoolean({ data: list, transFormFields: ['initEnd'], isCn: true }),
        ...
        'add': () => (
          ...
          this.formDialog.ruleForm = {
            ...new ruleForm(),
            authWarehouseCode: this.formDialog.ruleForm.authWarehouseCode
          }
        ),

        'update': () => {
          utools.titleCallBack.call(this,
            this.formDialog.clickRow,
            async () => {
              try {
                const { data } = await api.searchOnewarehouseauthorizationData(this.formDialog.clickRow.authWarehouseCode);
        ...
        'delete': () => {
          utools.titleCallBack.call(
            ...
            utools.removeReceiptsTips.bind(
              this,
              async () => {
                await api.deletewarehouseauthorizationData(this.formDialog.clickRow.authWarehouseCode);
  ...
  methods: {
      ...
      this.formDialog.validate(async valid => {
        if (valid) {
          utools.saveReceiptsTips.call(
            this,
            async () => {
              const { data } = await api.changeWarehouseauthorizationData({
    ...
    // 提交单据
    async submitStatus () {
      utools.sbumitReceiptsTips(
        this,
        async () => {
          await api.updateBillStatus({ type: 'submit', code: this.formDialog.ruleForm.authWarehouseCode });
    ...
    async loadCodeNo () {
      ...
      try {
        const {
          data
        } = await api.getwarehouseauthorizationCode()
        this.$set(this.formDialog.ruleForm, 'authWarehouseCode', data)
      } catch (e) {
