...
import tableconfig from '@/domain/tableconfig/basicdata/InspectionPurpose'
import ruleForm from '@/domain/entities/basicdata/InspectionPurpose'
import formConfig from '@/domain/formconfig/basicdata/InspectionPurpose'
...
export default {
  ...
  computed: {
    ...
    queryBarFormConfig () {
      return [
        { label: "编码", moduleBind: "inspectionPurposeCode", isInput: true },
    ...
    clickTypeAsync () {
      return {
        'search': async (page) => {
          try {
            const { data: { list, count } } = await api.queryinspectionpurposeSearch({ ...this.ruleForm, ...page });
            list && (this.utools.turnCodeBoolean(list), this.tableData = list, this.tableCount = count);
          } catch (e) { console.log(e) }
        },
        'add': () => (this.formDialog.isShowPopover = true, this.loadCodeNo(), this.formDialog.ruleForm = { ...new ruleForm(), inspectionPurposeCode: this.formDialog.ruleForm.inspectionPurposeCode }),
        'update': () => {          this.utools.titleCallBack.bind(this)(this.formDialog.clickRow, async () => {
            try {
              const { data } = await api.searchOneinspectionpurposeData(this.formDialog.clickRow.inspectionPurposeCode);
        ...
        'delete': () => {
          this.utools.titleCallBack.bind(this)(this.formDialog.clickRow, () => {
            this.utools.deleteMessage.bind(this)(async () => {
              try {
                const { status, data: { list, count } } = await api.deleteinspectionpurposeData(this.formDialog.clickRow.inspectionPurposeCode);
  ...
  methods: {
    ...
    async saveForm () {
      if (this.isVerify()) {
        try {
          let apiType = this.formDialog.btnKey === 'add' ? 'addinspectionpurposeData' : 'editinspectionpurposeData';
          const {
            data,
            status
    ...
    async loadCodeNo () { //加载编号
      ...
      try {
        const {
          data
        } = await api.getinspectionpurposeCode()
        this.formDialog.ruleForm.inspectionPurposeCode = data;
      } catch (e) { console.log(e) }
    },
  },
  created () {
