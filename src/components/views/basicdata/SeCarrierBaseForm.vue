...
import carrierBankTable from '@/domain/tableconfig/basicdata/CarrierBankTrial.js'
...
export default {
  ...
  computed: {
    ...
    // 表格按钮胡显示控制
  showBtn(){
    return this.tabActiveName === 'finance' ? true :( (+(this.form.data.base.status) || 0) <= 1)
  },
    isShowTable () {
  ...
  methods: {
    ...
    handlerTableRemove: (() => {
      ...
      return function () {
        if(this.tabActiveName === 'finance' && this.table.clickRow.status === '4')  return this.$message.warning('无法删除已审核的银行信息！')
        utools.titleCallBack.call(
          this,
          this.table.clickRow,
