...
import tableconfig from '@/domain/tableconfig/basicdata/Institution'
import ruleForm from '@/domain/entities/basicdata/Institution'
import formConfig from '@/domain/formconfig/basicdata/Institution'
...
export default {
  ...
  computed: {
    ...
    queryBarFormConfig () {
      return [
        { label: "编码", moduleBind: "institutionCode", isInput: true },
    ...
    clickTypeAsync () {
      return {
        'search': async (page) => {
          try {
            const { data: { list, count } } = await api.queryinstitutionSearch({ ...this.ruleForm, ...page });
            list && (this.utools.turnCodeBoolean(list), this.tableData = list, this.tableCount = count);
          } catch (e) { console.log(e) }
        },
        'add': () => (this.formDialog.isShowPopover = true, this.loadCodeNo(), this.formDialog.ruleForm = { ...new ruleForm(), institutionCode: this.formDialog.ruleForm.institutionCode }),
        'update': () => {          this.utools.titleCallBack.bind(this)(this.formDialog.clickRow, async () => {
            try {
              const { data } = await api.searchOneinstitutionData(this.formDialog.clickRow.institutionCode);
        ...
        'delete': () => {
          this.utools.titleCallBack.bind(this)(this.formDialog.clickRow, () => {
            this.utools.deleteMessage.bind(this)(async () => {
              try {
                const { status, data: { list, count } } = await api.deleteinstitutionData(this.formDialog.clickRow.institutionCode);
  ...
  methods: {
    ...
    async saveForm () {
      if (this.isVerify()) {
        try {
          let apiType = this.formDialog.btnKey === 'add' ? 'addinstitutionData' : 'editinstitutionData';
          const {
            data,
            status
    ...
    async loadCodeNo () { //加载编号
      ...
      try {
        const {
          data
        } = await api.getinstitutionCode()
        this.formDialog.ruleForm.institutionCode = data;
      } catch (e) { console.log(e) }
    },
  },
  created () {
