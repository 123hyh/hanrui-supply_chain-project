...
<template>
  <div class="customs-form-base">
    ...
    <div>
      ...
      ></form-component>
      <!-- 页签 -->
      <el-tabs @tab-click="handlerTabClick" type="card" v-model="activeName">
        <el-tab-pane
      ...
      <el-tabs @tab-click="handlerTableClick" type="card" v-model="table.tableName">
        <el-tab-pane
          :key="item.name"
          :label="item.label"
          :name="item.name"
        ...
        ></el-tab-pane>
        <div class="mb-10" v-show="(+CustomsHead.status||0)<=1">
          <!-- 商品信息的 按钮 去除 新增 -->
          <el-button
            :key="item.key"
            :type="item.type"
...
import customsFormBaseConfig from '@/domain/formconfig/custom/CustomsFormBase.js'
import customsConfig from '@/domain/formconfig/custom/CustomsOther.js'
import customsInspectionConfig from '@/domain/formconfig/custom/CustomsInspection.js'
...
export default {
  ...
  data() {
    return {
      ...
      status: '',
      activeName: 'formbase',
      CustomsHeadKey: utools.cloneObj(CustomsHeadKey),
  ...
  computed: {
    ...
    inspectionTabDisabled() {
      return !(this.status === 'update' && this.otherIsSave)
    },
    tabObj() {
      return [
        { name: 'formbase', label: '基本资料' },
        { name: 'other', label: '其他信息', disabled: this.otherTabDisabled },
        // { name: 'declaration', label: '报关信息' },
        {
          name: 'inspection',
          label: '报关报检信息',
          disabled: this.inspectionTabDisabled
        }
      ]
    },
    otherTabObj: () => [
    ...
    clickTypeAsync() {
      return {
        search: async () => {
          const {
            data: { list, count }
          } = await api.querySearch(`/customs${this.formDialog.otherTabName}`, {
            ...this.formDialog.paper,
            customsFormCode: this.CustomsHead.customsFormCode
          })
          list &&
            (this.utools.turnCodeBoolean(list),
            (this.formDialog.otherTabDate = list),
            (this.formDialog.paper.tableCount = count))
        },
        add: () => {
          this.formDialog.isShowPopover = true
          this.formDialog.ruleForm = {
            ...new OtherTabFormData(),
            customsFormCode: this.CustomsHead.customsFormCode
          }
        },
        edit: () => {
          this.utools.titleCallBack.bind(this)(
            this.formDialog.clickRow,
            async () => {
              try {
                const { data } = await api.searchOneData(
                  `/customs${this.formDialog.otherTabName}`,
                  { itemCode: this.formDialog.clickRow.itemCode }
                )
            ...
            this.utools.deleteMessage.bind(this)(async () => {
              try {
                const {
                  ...
                  `/customs${this.formDialog.otherTabName}`,
                  this.formDialog.clickRow.itemCode
                )
    ...
    clickTypeGoodsAsync() {
      return {
        search: async () => {
          const {
            data: { list, count }
          } = await api.querySearch(`/${this.table.tableName}`, {
            ...this.formDialog.paper,
            customsFormCode: this.CustomsHead.customsFormCode
          })
          list &&
            ...
            (this.table.tableData =
              this.table.tableName === 'goodsdetail'
                ? list.map((item, i) => {
                    item.index =
                      ((this.formDialog.paper.pageIndex || 1) - 1) *
                        (this.formDialog.paper.pageSize || 10) +
                      ++i
                    return item
                  })
                : list),
        ...
        edit: () => {
          this.utools.titleCallBack.bind(this)(
            this.table.clickRowData,
            async () => {
              try {
                const { data } = await api.searchOneData(
                  `/${this.table.tableName}`,
                  { itemCode: this.table.clickRowData.itemCode }
                )
            ...
            this.utools.deleteMessage.call(this, async () => {
              try {
                const {
                  ...
                  `/${this.table.tableName}`,
                  this.table.clickRowData.itemCode
                )
  ...
  methods: {
    ...
    setCnFieldName() {
      ...
      if (
        this.CustomsHead.customsType == '3' ||
        this.CustomsHead.customsType == 4
      ) {
        this.CustomsHead.customsType = '1'
        this.utools.alertStatusMessage.call(this, {
          msg: { warning: '不可选择中国金内外' }
        })
        return
      }
    ...
    handleDeletegoodsdetail() {
      ...
      let alert = {
        msg: { warning: '请选择一条数据！', success: '删除成功！' }
      }
      if (!!~i) {
    ...
    handleBtnClick: (() => {
      //点击查询栏按钮事件
      var obj = null
      return function(type, clear, page) {
        // console.log(type, clear)
        if (clear) {
          obj = null
        } else {
          !obj && (obj = this.handleBtnClickObj())
          obj[type].call(this, page)
        }
      }
    })(),
    handleBtnClickObj() {
    ...
    handlerSubPreservation(rowData, key) {
      if (this.tabForm[key] !== undefined) {
        if (key === 'contractNo') {
          ...
          ;(this.tabForm[key] = rowData['companyName']),
            (this.tabForm['homeShipper'] = rowData['companyCode'])
        } else {
      ...
      if (this.formDialog.ruleForm[key] !== undefined) {
        if (key === 'containerSpecification' || key === 'containerCode') {
          ...
          this.formDialog.ruleForm[key] = rowData.map(
            item => item.entrustOrderNo
          )
          ...
          this.formDialog.ruleForm['desCountryCode'] =
        ...
        // 报关计划单请求商品信息 和
        Promise.all([
          this.requestCommodity(rowData[key]),
          this.setFormTranstion(rowData)
        ])
      } else if (key === 'superConditions') {
        this.formDialog.ruleForm = {
          ...this.formDialog.ruleForm,
          superConditions: rowData['billName'],
          superConditionsCode: rowData['billCode']
        }
      } else if (key === 'originPlaceName') {
        this.formDialog.ruleForm = {
          ...this.formDialog.ruleForm,
          originPlaceName: rowData['originName'],
          originPlaceCode: rowData['originCode']
        }
      } else if (key === 'desCountryName') {
        this.formDialog.ruleForm = {
          ...this.formDialog.ruleForm,
          desCountryName: rowData['arrivalCountryName'],
          desCountryCode: rowData['arrivalCountryCode']
        }
      }
      this.$forceUpdate()
    },
    ...
    async requestCommodity(planOrderNo) {
      ...
      try {
        const { data, status } = await api.getImportCustomGoodsList({
          planOrderNo,
          customsFormCode: this.CustomsHead.customsFormCode
        })
        this.table = {
          ...
          tableData: this.utools.cloneObj(data.slice(0, 10))
        }
      } catch (error) {
        console.log(error)
      }
    ...
    rousePopover(key) {
      ...
      if (key === 'planOrderNo') {
        ...
        let thisKey = unitArr.includes(key)
          ? 'busiUnit'
          : countryArr.includes(key)
          ? 'originCountry'
          : officeArr.includes(key)
          ? 'iqOffice'
          : key
        this.popoverParamsFn(popover[thisKey])
      } /* if (this.formDialog.ruleForm[key] !== undefined)  */ else {
      ...
      let commonSearch = {
        companyName: [
          { label: '名称', moduleBind: 'companyName', isInput: true }
        ],
        origin: [{label: '国家/代码',moduleBind: 'keyword',isInput: true}],
        institution:  [{label: '名称',moduleBind: 'institutionName',isInput:true}]
      }

      let tableDialogSearchConfig = {
        destination: [
          {
            label: '地区名称',
          ...
          { label: '地区代码', moduleBind: 'customsCode', isInput: true }
        ],
        packagingType: [
          { label: '名称', moduleBind: 'packageTypeName', isInput: true }
        ],
        overseaShipper: commonSearch.companyName,
        homeShipperName: commonSearch.companyName,
        busiUnit: commonSearch.companyName,
        receiveUnit: commonSearch.companyName,
        storagePlace: commonSearch.companyName,
        exemption: [{label: '全称',moduleBind:'exemptionName',isInput: true}],
        tradingCountry: commonSearch.origin,
        originCountry: commonSearch.origin,
        declarationUnit: commonSearch.companyName,
        destinationOffice:commonSearch.institution,
        portOffice: commonSearch.institution,
        iqOffice: commonSearch.institution,
        licenseOffice: commonSearch.institution
      }
      // 设置查询条件 end  ↑
      ...
      let currencyObj = {
        ...
        [key]: async () => {
          this.popover = {
            ...this.popover,
            queryBarObj: tableDialogSearchConfig[key]
              ? tableDialogSearchConfig[key]
              : [
                  {
                    label: '编码',
            ...
            popoverListKey: require('@/domain/tableconfig/' +
    ...
    handlePageChange(paper) {
      ...
      this.handleBtnClick('search', '', paper) //翻页查询
    },
    /************************** 弹窗methods end *******************/
    async handlerTabClick({ name } = tab, event) {
      ...
      try {
        ...
        const formObj = {
          formbase: customsFormBaseConfig,
          other: customsConfig,
          inspection: customsInspectionConfig
        }
        this.formConfig = utools.cloneObj(
          formObj[name]
        ) /* utools.cloneObj( require('@/domain/formconfig/custom/Customs' + newValCase).default) */
        api.initSelect(this.formConfig)
        TabForm = require('@/domain/entities/custom/Customs' + newValCase)
          .default
        ...
        if (this.status !== 'add' && isNull) {
          const {
            ...
            '/customs' + (name == 'declaration' ? 'inspection' : name),
            { customsFormCode: this.CustomsHead.customsFormCode }
          )
          ...
          let isNull = !!Object.keys(this.tabData[this.activeName]).length
          isNull &&
            (this.tabForm = this.utools.cloneObj(this.tabData[this.activeName]))
        }
        // 子主键
        this.customsCode =
    ...
    // 切换子表页签
    async handlerChildTabClick({ name } = tab, event) {
      OtherTabFormData = require('@/domain/entities/custom/Customs' +
        this.utools.capitalize(name)).default
      this.formDialog.ruleForm = {
        ...new OtherTabFormData(),
        customsFormCode: this.CustomsHead.customsFormCode
      }
      this.formDialog.formConfig = require('@/domain/formconfig/custom/Customs' +
        this.utools.capitalize(name)).default
      this.formDialog.otherTabTableConfig = require('@/domain/tableconfig/custom/Customs' +
        this.utools.capitalize(name)).default
      api.initSelect(this.formDialog.formConfig)
      this.clickTypeAsync['search']()
      // this.formDialog.clickRow = {}; // 清空当前行数据
    },
    async handlerTableClick({ name } = tab, event) {
      name = name == 'goodsdetail' ? 'goodsDetail' : name
      RuleForm = require('@/domain/entities/custom/' +
        this.utools.capitalize(name)).default
      let formDialogIsNull = !!Object.keys(this.formDialog.ruleForm)
      !formDialogIsNull &&
        (this.formDialog.ruleForm = {
          ...new RuleForm(),
          customsFormCode: sessionStorage.customsFormCode
        })
      this.formDialog.formConfig = require('@/domain/formconfig/custom/' +
        this.utools.capitalize(name)).default
      this.table.tableConfig = require('@/domain/tableconfig/custom/' +
        this.utools.capitalize(name)).default
      api.initSelect(this.formDialog.formConfig)
    ...
    async handlerPreserve() {
      const { status } = this
      let formData = {
        ...this.CustomsFormBase,
        /* ...this.CustomsHead,  */ customsFormCode: this.CustomsHead
          .customsFormCode
      }
      for (let key in this.CustomsHead) {
        // 这步是为了避免该集合数据被覆盖
        this.CustomsHead.hasOwnProperty(key) &&
          !this.CustomsFormBase[key] &&
          (formData[key] = this.CustomsHead[key])
      }
      ...
      if (this.isVerify() && this.isTabVerify()) {
        ...
        try {
          let apiType =
            this.status === 'add'
              ? 'addcustomsformbaseData'
              : 'editcustomsformbaseData'
          const { data, status } = await api[apiType](formData)
    ...
    async saveTab() {
      if (this.isVerify() && this.isTabVerify()) {
        ...
        try {
          ...
          const { data, status } = await api.addData(
            `/customs${this.activeName}`,
            formData
          )
          statusMessage = status
    ...
    openTablePopForm(btnKey, tabString) {
      ...
      if (this.status === 'add' && btnKey !== 'delete') {
        ...
        ;(this.formDialog.ruleForm = this.utools.cloneObj(
          this.table.clickRowData
        )),
          (this.formDialog.isShowPopover = true)
        return
    ...
    async handlerOtherSubmit() {
      if (this.status === 'add' && this.table.tabString === 'goodsdetail') {
        ...
        try {
          let apiType =
            this.formDialog.btnKey === 'add' ? 'addData' : 'editData'

          // tabString 为 goodsdetail（字表 商品信息）和其他 页签不同
          let apiUrl =
            this.table.tabString === 'goodsdetail'
              ? this.formDialog.btnKey === 'add'
                ? '/goodsdetail/insertOrderNo'
                : `/${this.table.tableName}`
              : `/customs${this.formDialog.otherTabName}`

          let { data, status } = await api[apiType](
            apiUrl,
            this.formDialog.ruleForm
          )
          this.formDialog.isShowPopover = false
    ...
    async loadCodeNo() {
      ...
      // 新增时获取采购订单编号
      try {
        const { data } = await api.getcustomsformbaseCode()
        // console.log(data)
    ...
    async initData() {
      ...
      ;(this.CustomsHead = CustomsHead),
        (this.CustomsFormBase = CustomsFormBase)
      this.status = status
      tabData && (this.tabData = tabData) // 页签的数据和控制
      if (status === 'add' && !this.CustomsHead.customsFormCode) {
        this.loadCodeNo()
      } else {
        // 获取其他信息的 是否报检 来启用报检页签
        if (this.tabData.other.needInspection === undefined) {
          const {
            data: {
              list: [{ needInspection = false }]
            }
          } = await api.getOtherData({
            customsFormCode: CustomsHead.customsFormCode
          })
          this.otherIsSave = needInspection
        } else {
          this.otherIsSave = this.tabData.other.needInspection
        }
      }
  ...
  beforeRouteLeave(to, form, next) {
    ...
    let obj = {
      data: {
        formData: {
          CustomsHead: this.CustomsHead,
          CustomsFormBase: this.CustomsFormBase,
    ...
    if (this.status === 'add')
      obj.data.formData.goodsdetail = this.table.tempData
