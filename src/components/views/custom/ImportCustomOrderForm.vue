<template>
  <div class="Box import-custom-order-form">
    ...
    <!-- 表格 -->
    <div>
      <el-button @click="openTabForm()" v-show="(+form.data.status || 0) < 1">新增</el-button>
    </div>
    <section v-show="table.isshow">
      ...
      ></table-component>
      <pagination-component :count="table.pageCount" @handlePageChange="handleTablePageChange"></pagination-component>
    </section>
...
export default {
  ...
  computed: {
    ...
    calculateItem() {
      return {
        targetCurrencyToRmb:
          (this.formDialog.ruleForm.originalCurrencyToRmb || 0) *
          (this.formDialog.ruleForm.quantity || 0) // 目标币到人民币 = 数量 * 原币别到人民币
      }
    },
    DialogRequest() {
      return {
    ...
    tableConfigs: () => ({
      waitCustomList: {
        ...
        config: _ =>
          require('@/domain/tableconfig/logistics/Precustoms.js').default
      },
      businessUnitName: {
        title: '公司列表',
      ...
      transportMachine: {
        ...
        config: _ =>
          require('@/domain/tableconfig/basicdata/Conveyance.js').default
      },
      deliveryOrderNo: {
        title: '香港上货计划',
      ...
      carNumber: {
        ...
        config: _ =>
          require('@/domain/tableconfig/basicdata/TrainNumber.js').default
      },
      packageKindName: {
        title: '包装种类',
        searchCode: 'packageCode',
        config: _ =>
          require('@/domain/tableconfig/basicdata/PackageType.js').default
      },
      entrustOrderNo: {
        title: '委托订单',
      ...
      superviseTerm: {
        ...
        searchCode: 'billCode',
        config: _ =>
          require('@/domain/tableconfig/basicdata/SupervisionCondition.js')
            .default
      },
      exemption: {
        title: '征免性质',
        ...
        config: _ =>
          require('@/domain/tableconfig/basicdata/Exemption.js').default
      },
      superviseMode: {
        title: '监管方式',
        searchCode: 'supervisionModeCode',
        config: _ =>
          require('@/domain/tableconfig/basicdata/SupervisionMode.js').default
      },
      unitName: {
        title: '计量单位',
    ...
    clickTypeGoodsAsync() {
      return {
        ...
        edit: () => {
          this.utools.titleCallBack.bind(this)(this.clickRowData, async () => {
            try {
              const { data } = await api.searchOneData(`/${this.tableName}`, {
                itemCode: this.clickRowData.itemCode
              })
            ...
            this.utools.deleteMessage.bind(this)(async () => {
              try {
                var {
                  status,
                  data: { list, count }
                } = await api.deleteimportcustomgoodsData(
  ...
  methods: {
    ...
    async getsolutionformula() {
      try {
        ...
        this.utools.setCalcconfigObj(this.formDialog.formConfig, data.list)
      } catch (error) {
        console.log(error)
      }
    },
    ...
    async initTableData(page = {}) {
      try {
        const {
          data: { list, count }
        } = await api.queryimportcustomgoodsSearch({
          ...page,
          planOrderNo: this.form.data.planOrderNo
        })
    ...
    async handlerFormConfigClickSearch(params, obj, queryData) {
      ...
      this.tableDialog.popoverType =
        params == 'orderNos' || params == 'entrustOrderNo'
          ? 'selectTable'
          : 'table' // 委托单号是多选表格
      // 弹窗条件
      // queryData = params == 'goodsName' ? { ...queryData, entrustOrderNos: [this.formDialog.ruleForm.orderNos] } : queryData;
      // queryData.matPayment = params == 'entrustOrderNo' ? true : '';
      try {
        const {
          data: { list, count }
        } = await this.DialogRequest[params]({
          ...queryData,
          ...this.setReqStatus()
        })
        this.tableDialog = {
    ...
    setReqStatus() {
      ...
      const STATUS_4 = ['entrustOrderNo']
      // 设置请求状态参数
      let param = { status: '' }
      if (WHITE_LIST.includes(this.dialogTarget)) {
        param.status = '2'
      } else if (STATUS_4.includes(this.dialogTarget)) {
        param.status = '4'
      }
      return param
    },
    ...
    tableDialogPreservation(data) {
      switch (this.dialogTarget) {
        case 'waitCustomList':
          this.form.data = {
            ...this.form.data,
            waitCustomList: data.preCustomsCode,
            flightNo: data.voyageNo,
            grossWeight: data.grossWeight,
            carSort: data.vehicleSequence
          }
          break
        case 'businessUnitName':
        case 'declarationUnitName':
        ...
        case 'transportMachine':
          this.form.data = {
            ...this.form.data,
            transportMachine: data.conveyanceName,
            transportMachineCode: data.conveyanceCode
          }
          break
        case 'deliveryOrderNo':
          this.form.data = {
            ...this.form.data,
            deliveryOrderNo: data.loadingBillNo,
            carNumber: data.trips,
        ...
        case 'scheduleBaseCode':
          this.form.data = {
            ...this.form.data,
            scheduleBaseCode: data.scheduleBaseCode
          }
          this.form.data = {
            ...this.form.data,
            deliveryOrderNo: data.loadingBillNo,
        ...
        case 'carNumber':
          this.form.data = {
            ...this.form.data,
            carNumber: data.trainNumberCode
          }
          break
        case 'packageKindName':
          this.form.data = {
            ...this.form.data,
            packageKind: data.packageTypeCode,
            packageKindName: data.packageTypeName
          }
          break
        case 'entrustOrderNo':
          this.form.data = {
            ...
            customType: data.reportGateType
          }
        ...
        case 'goodsName':
          this.formDialog.ruleForm = {
            ...
            /* goodsName: data.goodsName, goodsCode: data.goodsCode, */ goodsModel:
              data.arrivalGoodsModel
          }
          break
        case 'taxNo':
          this.formDialog.ruleForm = {
        ...
        case 'sourceArea':
          this.formDialog.ruleForm = {
            ...this.formDialog.ruleForm,
            sourceArea: data.originName,
            sourceAreaCode: data.originCode
          }
          break
        case 'superviseTerm':
          this.formDialog.ruleForm = {
            ...this.formDialog.ruleForm,
            superviseTerm: data.billName,
            superviseTermCode: data.billCode
          }
          break
        case 'exemption':
          this.formDialog.ruleForm = {
            ...this.formDialog.ruleForm,
            exemption: data.exemptionName,
        ...
        case 'superviseMode':
          this.formDialog.ruleForm = {
            ...this.formDialog.ruleForm,
            superviseMode: data.supervisionModeName,
            superviseModeCode: data.supervisionModeCode
          }
          break
        // 计量单位
        case 'unitName': {
    ...
    async getEntrustOrdersGoodsList(val) {
      ;(this.table.tempData = []), (this.table.pageCount = 0)
      try {
        const { data = [] } = await api.getEntrustOrdersGoodsList(
          val.map(item => item.entrustOrderNo)
        )
        this.table = {
          ...this.table,
      ...
      if (this.status === 'add') {
        ...
        if (type !== 'delete') {
          if (type === 'add') {
            ...
            try {
              const { data } = await api.searchOneimportcustomgoodsData(
                this.clickRowData.itemCode
              )
              this.formDialog.ruleForm = this.clickRowData = {
                ...data,
                orderNos: JSON.stringify([data.orderNo]).replace(/\[|]|"/g, '')
              }
            } catch (e) {
              console.log(e)
            }
    ...
    handleTablePageChange(pageData = {}) {
      if (this.status === 'add') {
        ...
        this.table.tableData = this.utools.cloneObj(
          tempData.slice(index, pageSize)
        )
        return
      }
    ...
    async handlePreserve() {
      ...
      try {
        ...
        IS_ADD &&
          ((data.importCustomGoods = []),
          (data.importCustomGoods = this.table.tempData))
        const { status } = await api.changeImportCustomOrderData({
          data,
    ...
    async handlerTableFormPreservation() {
      if (this.status === 'add') {
        ...
        if (!~tableDataIndex)
          return this.$message({ type: 'warning', message: '没有该条商品！！' })
        ~tableDataIndex &&
          (this.table.tableData[tableDataIndex] = {
            ...this.formDialog.ruleForm,
            ...this.calculateItem
          })
        ~tempDataIndex &&
          (this.table.tempData[tempDataIndex] = {
            ...this.formDialog.ruleForm,
            ...this.calculateItem
          })
        ...
        this.utools.alertMessage.bind(this)(status, message)
        if (status == 200) this.formDialog.isshow = false
      }
    },
    handlebackspacing() {
      this.$router.push({
    ...
    initData() {
      const { formData = {}, status, table = {} } = this.getActivePathData(
        this.$route.path
      )
      this.status = status
      if (this.status === 'add') {
        ...
        table.tempData &&
          (this.table.tableData = this.utools.cloneObj(
            this.table.tempData.slice(0, 10)
          ))
