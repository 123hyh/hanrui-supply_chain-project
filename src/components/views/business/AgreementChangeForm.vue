<template>
	<div>
		...
		<preserve-btn @handlePreserve='handlePreserve' @handleRollback='handleSubmit' :isLoading='isLoading'></preserve-btn>
		<!-- 输入框 -->
		<div>
			<form-config 
		...
		<el-row :gutter="10">
			<el-col :span="12">
				<el-tabs v-model="tabActiveName_one" class="handleTableHeight" type="border-card" @tab-click="handlerTabClick_one">
					<el-tab-pane label="方案明细" name="solution" :disabled='tabIsDisabled_one'></el-tab-pane>
					<el-tab-pane label="合同条款明细" name="contract" :disabled='tabIsDisabled_one'></el-tab-pane>
					<!-- <el-tab-pane label="被修改日志记录" name="record" :disabled='tabIsDisabled_one'></el-tab-pane> -->
					<el-tab-pane label="授权邮箱" name="email" :disabled='tabIsDisabled_one'></el-tab-pane>
					<!-- 表格 -->
					<div v-if="!tabIsDisabled_one">
						<div v-if="tabActiveName_one=='contract'">
							<form-config 
								...
								<!-- 按钮行 -->
								<div slot="footer">
									<el-button type="primary" @click="saveTab_one('contract')">保存</el-button>
								</div>
						</div>
						<div v-if="tabActiveName_one!='contract'">
							<div class="mb-10" v-if="tabActiveName_one!='record'">
								<el-row>
									<el-col :span="24">
										<!-- 操作 -->
										<el-button type="primary" @click="openTabForm_one('create')">新增</el-button>
										<el-button type="primary" @click="openTabForm_one('update')">修改</el-button>
										<el-button type="danger" @click="openTabForm_one('delete')">删除</el-button>
									</el-col>
								</el-row>
							</div>
							<el-table 
								...
								@row-click="clickTabRow_one">
								<el-table-column v-for='item in columnDataTable_one' :type="item.type" :key="item.label" :prop="item.prop" :label="item.label" :width="item.width"></el-table-column>
							</el-table>
				
							<!-- 分页 快捷键：ctrl+] 首尾加标签-->
							<pagination @handlePageChange="handleTabPageChange_one" :count="queryTableData_one.count" ></pagination>
						</div>
					</div>
				</el-tabs>
			</el-col>
			<el-col :span="12">
				<el-tabs v-model="tabActiveName" class="handleTableHeight" type="border-card" @tab-click="handlerTabClick">
					<el-tab-pane label="受托方收款项目" name="clinet" :disabled='tabIsDisabled'></el-tab-pane>
					<el-tab-pane label="商务条款" name="clause" :disabled='tabIsDisabled'></el-tab-pane>
					<el-tab-pane label="补充协议" name="replenish" :disabled='tabIsDisabled'></el-tab-pane>
					<el-tab-pane label="保险事项" name="insurance" :disabled='tabIsDisabled'></el-tab-pane>
					<!-- <el-tab-pane label="阶梯报价规则" name="offer" :disabled='tabIsDisabled'></el-tab-pane> -->
					<el-tab-pane label="供应商承担费用" name="supplier" :disabled='tabIsDisabled'></el-tab-pane>
					<!-- 表格 -->
					<div v-if="!tabIsDisabled">
						<div class="mb-10">
							<el-row>
								<el-col :span="24" v-if="tabActiveName!='offer'">
									<!-- 操作 -->
									<el-button v-if="(tabActiveName_one == 'solution'&&agreementSolutionNo)||tabActiveName != 'clinet'" type="primary" @click="openTabForm('create')">新增</el-button>
						...
						<!-- 分页 快捷键：ctrl+] 首尾加标签-->
						<pagination @handlePageChange="handleTabPageChange" :count="queryTableData.count" ></pagination>
					</div>
				</el-tabs>
			</el-col>
		</el-row>
		<!--弹出层： 供应链供应商form -->
		<el-dialog v-dialogDrag :visible.sync="popFormTab_one" :close-on-click-modal="false" width="53%" :title="title_one">
			<form-config 
				...
				<!-- 按钮行 -->
				<div slot="footer">
					<el-button type="primary" @click="saveTab_one">保存</el-button>
					<el-button type="cancel" @click="closeTabForm_one">取消</el-button>
				</div>
		</el-dialog>
		<!--弹出层： 供应链供应商form -->
		<el-dialog v-dialogDrag :visible.sync="popFormTab" :close-on-click-modal="false" width="53%" :title="title">
			<form-config 
		...
		methods:{
			...
			async handleTabPageChange_one(paper){
				...
				if(this.tabActiveName_one=='contract'&&this.dataTab_one.length){
					const { data: contractData } = await api.searchOneagreementcontractData(this.dataTab_one[0].itemCode);
			...
			async handlerTabClick({
				...
				this.title = name=='clinet'? '受托方收款项目': name=='clause'? '商务条款': name=='replenish'? '补充协议': name=='insurance'? '保险事项': name=='offer'? '阶梯报价规则': '供应商承担费用';
				ColumnData = require("@/domain/entities/business/Agreement" + this.capitalize(name)).default;
				...
				name!=='clinet'? this.handleTabPageChange({pageIndex: 1, agreementNo: this.agreementForm.agreementNo})
				: this.handleTabPageChange({pageIndex: 1, agreementNo: this.agreementForm.agreementNo, solutionNo: (this.agreementSolutionNo? this.agreementSolutionNo: '123456')});
			...
			async openTabForm_one(btnType) {
				...
				// 判断是不是方案明细
				let codeNo = this.tabActiveName_one == 'solution'? {solutionNo: clickRowData_one.solutionNo}: {itemCode: clickRowData_one.itemCode};
				if (btnType == 'update' && JSON.stringify(clickRowData_one)!='{}') {
					try{const {
							data: searchOneData
						} = await api.searchOneData('/agreement' + this.tabActiveName_one,codeNo)
				...
				if (btnType == 'delete' && JSON.stringify(clickRowData_one)!='{}') {
					this.utools.deleteMessage.bind(this)(async ()=>{
						let itemCode = this.tabActiveName_one=='solution'? clickRowData_one.solutionNo: clickRowData_one.itemCode;
						try {
			...
			async openTabForm(btnType) {
				...
				if (btnType == 'update' && JSON.stringify(this.clickRowData)!='{}') {
					try{const {
							data: searchOneData
						} = await api.searchOneData('/agreement' + this.tabActiveName,{itemCode: this.clickRowData.itemCode})
