...
//表格配置
import tableconfig from "@/domain/tableconfig/business/SellerReceiveGoods";
...
export default {
  ...
  methods: {
    ...
    async queryData () {
      try {
        const { data } = await api.sellerReceiveSearch(this.ruleForm);
              ...
              beforeClose: (action, instance, done) => {
                if (action === "confirm") {
                  ...
                  setTimeout(() => {
                    // 删除
                    api
                      .deleteSellerReceiveList(this.curRowData.billNo)
                      .then(res => {
                        if (res) {
