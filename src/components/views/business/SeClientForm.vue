...
export default {
  ...
  computed: {
    showBtn(){
      return this.tabActiveName == 'bank' || ( this.tabActiveName != 'agreement' && (this.clientForm.status == '1' || !this.clientForm.status))
    },
    requestAddress () {
      return {
  ...
  methods: {
    ...
    async handleTabPageChange (paper = {}) {
      ...
      const {
        ...
        this.tabActiveName == 'bank' ?
          api.getALLBankDataAll({ billNo: paper.clientNo, ...bankParams }) :
          api.querySearch(
    ...
    async handlerTabClick ({ name } = tab, event) {
      ...
      if(this.capitalize(name) == 'Bank'){
      this.columnDataTable = require("@/domain/tableconfig/business/Client" +
        this.capitalize(name)+'Trial').default;
      }
    ...
    async openTabForm (btnType) {
      ...
      if (btnType != "create" && JSON.stringify(this.curRowData) == "{}") {
        ...
        this.$set(this.columnData, 'clientNo',sessionStorage.clientNo)
        this.addBtnType(btnType);
        this.popFormTab = true;
      }
      // 确认删除
      if (btnType == "delete" && JSON.stringify(this.curRowData) != "{}") {
        if (this.curRowData.status === '4') return this.$message.warning('无法删除已审核的银行信息！')
