...
<template>
  <div>
    ...
    <div>
      <form-component
        :formModel='DispatchOrderForm'
        :inputKey='DispatchOrderFormKey'
      ...
      >
        ...
        <div v-if="!isTabDisabled">
          ...
          <table-component
            ...
            :isSubTable='true'
            :dialog='false'
            :popoverList="tableName != 'hasSelect' ? tableData : selectTabSearch"
            :popoverListKey='tableKey'
            :count='pageCount'
            :multipleTable.sync='selectTab'
            :activeRow.sync='fromDialogData.curRowData'
...
// 表单配置
import DispatchOrderForm from "@/domain/entities/logistics/DispatchOrder";
import DispatchOrderFormKey from "@/domain/formconfig/logistics/DispatchOrder";
...
export default {
  ...
  computed: {
    ...
    WHITE_LIST: _ => ([
      'plateNo', 'carrier', 'driver', 'stocker', 'shopper', 'shippingRoute'
    ]),
    tableObj: () => ([
  ...
  watch: {
    ...
    'DispatchOrderForm.outCar': {
      handler (cur, pre) {
        for (let item of this.DispatchOrderFormKey) {
          switch (item.key) {
            case 'carrier':
  ...
  methods: {
    ...
    handleBtnClickObj () {
      return {
        search: async () => {
          try {
            var { data: { count, list } } = await api.querySearch(this.popover.apiKey, {
    ...
    handlerSubPreservation (rowData, key) {
      if (this.DispatchOrderForm[key] !== undefined || this.fromDialogData.ruleForm[key] !== undefined) {
        if (key == "plateNo") {/* 车辆带出车型 */
          this.DispatchOrderForm[key] = rowData["vehicleNo"] || rowData["vehicleInfoCode"];
          this.DispatchOrderForm['plateNoCode'] = rowData["vehicleNo"] || rowData["vehicleInfoCode"];
          this.DispatchOrderForm['carType'] = rowData['carType'];
        } else if (key == "shippingRoute") {
          this.DispatchOrderForm[key] = rowData["shippingRouteName"];
          this.DispatchOrderForm['shippingRouteCode'] = rowData["shippingRouteCode"];
        } else if (key == "carrier") {
          this.DispatchOrderForm[key] = rowData["carrierName"];
          this.DispatchOrderForm['carrierCode'] = rowData["carrierCode"];
        ...
        } else if (key == "deliveryTrain") { // 配送车次
          this.handleRiteBackForm(rowData);
        } else if (key == "driver") {
          this.DispatchOrderForm[key] = rowData["driverName"];
          this.DispatchOrderForm['driverCode'] = rowData["driverInfoCode"] || rowData["driveNo"];
          this.DispatchOrderForm['driverPhone'] = rowData["phone"] || rowData["telePhone"];
        } else {
          this.DispatchOrderForm[key] = rowData["employeeInfoName"];
          this.DispatchOrderForm["shopperCode"] = rowData["employeeInfoCode"];
        }
      }
    },
    // 配送车次 回写表单 字段 transtion
    handleRiteBackForm (data = {}) {
      const { deliveryTrain, distributionTrainCode, loadWeight, carrier, driver, telePhone, packageNo, chargingScheme, shopper, shopperCode, plateNo } = data;
      this.DispatchOrderForm = {
        ...this.DispatchOrderForm,
        deliveryTrain,
        deliveryTrainCode: distributionTrainCode,
        capacity: loadWeight,
        carrier: carrier,
        driver: driver,
    ...
    rousePopover (key) {
      if (this.DispatchOrderForm[key] !== undefined || this.fromDialogData.ruleForm[key] !== undefined) {
        ...
        if (this.DispatchOrderForm['outCar']) {
      ...
      let currencyObj = {
        ...
        [key]: async () => {
          try {
            let params = this.DispatchOrderForm['outCar'] ? { carrierCode: this.DispatchOrderForm["carrierCode"] } : {};
    ...
    handlerPreserve () {
      this.verify['formModel'].validate(async (valid) => {
        if (valid) {
          ...
          try {
            let apiType = this.status === 'add' ? 'adddispatchorderData' : 'editdispatchorderData';
            const {
              data,
              status
            } = await api[apiType](this.DispatchOrderForm);
            this.DispatchOrderForm = { ...this.DispatchOrderForm, status: '1' }
    ...
    handlerTabBtnClick (key) {
        ...
        'create': async () => {
          if (this.selectTab.length == 0) {   //生成数据
            ...
            for (let item of this.selectTab) {
              item.dispatchOrderCode = this.DispatchOrderForm.dispatchOrderCode
            ...
            const { data } = await api.insertBatchdispatchentrustData(this.selectTab);
            this.tabRequest();
          }
        },
      }
      ...
      this.fromDialogData.ruleForm.billNo = this.DispatchOrderForm.billNo;
      this.setCurrent('');
    },
    ...
    async loadCodeNo () { //加载编号
      ...
      if (!saveObj) {
        try {
          const {
            data
          } = await api.getdispatchorderCode()
          this.DispatchOrderForm.dispatchOrderCode = data;
          this.DispatchOrderForm.orderStatus = '1';
        } catch (e) { console.log(e) }
      }
    },
    // 加载列表
    async tabRequest (params = {}) {
      ...
      try {
        if (!this.DispatchOrderForm.dispatchOrderCode) return;
        const {
          data: {
            list,
            count
          }
        } = await api.querydispatchentrustSearchEntrusts({ ...params });
        this.tableData = list, this.pageCount = count;
      } catch (e) { console.log(e) }
    },
    async selectRequest (params = {}) {
      this.selectTabSearch = []
      let obj = this.DispatchOrderForm.dispatchOrderCode;
      const {
        data: {
          list,
          count
        }
      } = await api.querydispatchentrustSearch({        ...params,
    ...
    async initData () {
      ...
      let obj = utools.getSession('dispatchOrderForm');
      this.status == 'update' ? (
        this.DispatchOrderForm = obj
      ) : await this.loadCodeNo();
      this.tableName = 'entrust';
      this.DispatchOrderForm.operator = window.localStorage.getItem("ms_username");
      if (this.status == 'add') {
        this.DispatchOrderForm.billStatus = '1';
        this.DispatchOrderForm.operateTime = this.currentTime;
        let saveObj = utools.getSession('saveMark')
        ...
        this.DispatchOrderForm.billStatus = obj.billStatus;
        this.DispatchOrderForm.operateTime = obj.operateTime;
        this.isTabDisabled = false;
      }
      this.tabRequest();
    },
  ...
  created () {
    ...
    formData && (this.DispatchOrderForm = formData);
    this.status = status;
    this.initData();
  },
  beforeRouteLeave (to, form, next) {
    this.addbreadCrumbsList({
      data: { formData: this.DispatchOrderForm, status: this.status },
