  ...
  <div class="table">
    ...
    <div class="container pd-10">
      ...
      <pagination-component
        ...
        @handlePageChange='queryHkschedulebase'
      ></pagination-component>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    queryHkschedulebase (page = {}) {
      appHelper.queryHkschedulebase(page).then(result => {
    ...
    queryData () {
      this.queryHkschedulebase(this.queryBar.data)
    },
    goDelete () {
      if (this.currentRow && this.currentRow.scheduleBaseCode) {
        this.$confirm('此操作将永久删除单据, 是否继续?', '提示', { type: 'warning' }).then(() => {
          appHelper.removeHkschedulebase(this.currentRow.scheduleBaseCode).then(data => {
            this.queryHkschedulebase()
    ...
    jumpForm (type) {
      ...
      type == 'update' && (params.data.formData.HkscheduleBaseEntity = this.currentRow)
