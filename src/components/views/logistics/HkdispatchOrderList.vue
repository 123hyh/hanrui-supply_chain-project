...
import tableconfig from '@/domain/tableconfig/logistics/HkdispatchOrder'
...
export default {
  ...
  computed: {
    ...
    clickTypeAsync () {
      return {
        'search': (data = {}) => api.queryhkdispatchorderSearch({ ...data, ...this.ruleForm }),//data:数据，ruleForm:分页；
        'add': () => { utools.setSession('hkdispatchOrderForm', { jumpType: 'add' }); this.jumpForm('add') },
        'delete': () => { if (this.clickRow.dispatchOrderCode !== undefined) return api.deletehkdispatchorderData(this.clickRow.dispatchOrderCode) },
        'update': () => {          this.titleCallBack(async () => {
            const { data } = await api.searchhkdispatchorderOneData(this.clickRow.dispatchOrderCode);
            // console.log(data)
            utools.setSession('hkdispatchOrderForm', data)
      ...
      if (clickType) {
        if (clickType === 'update' || clickType === 'add') {
          ...
          if (clickType === 'delete') {
            ...
            const { status, data: { list, count } } = await this.clickTypeAsync[clickType](page);
            list && (this.tableData = list, this.tableCount = count, utools.turnCodeBoolean(this.tableData), utools.splitDate(this.tableData, ['deliveryTime', 'operateTime', 'stockTime']));
          }
        }
      }
    },
