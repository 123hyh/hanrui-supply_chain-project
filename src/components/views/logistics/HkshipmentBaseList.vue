...
import tableConfig from "@/domain/tableconfig/logistics/HkshipmentBase.js";
// 组件
...
export default {
  ...
  methods: {
    ...
    querySzinstore (page = {}) {
      appHelper.queryHkshipmentbase(page).then(result => {
    ...
    goDelete () {
      if (this.currentRow && this.currentRow.shipmentBaseCode) {
        this.$confirm("此操作将永久删除单据, 是否继续?", "提示", {
          ...
          .then(() => {
            appHelper
              .removeHkshipmentbase(this.currentRow.shipmentBaseCode)
              .then(data => {
                this.querySzinstore();
    ...
    jumpForm (type) {
      ...
      this.$router.push(`HkshipmentBaseForm`);
    },
    closeForm () {
      this.popForm = false;
    },
