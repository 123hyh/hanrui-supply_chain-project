...
// 注册表
import formConfig from "@/domain/formconfig/logistics/ReceiptOrder.js";
import tableConfig from "@/domain/tableconfig/logistics/ReceiptBase.js";
import itemFormConfig from "@/domain/formconfig/logistics/ReceiptBase.js";
...
export default {
  ...
  watch: {
    "form.data.sourceBillType" (cur, pre) {
      ...
      let pam = { delegete: '', delegeteName: '', supplier: '', supplierName: '', client: '', clientName: '', reportGateType: '', issueType: '', issueTypeName: '', storeOrg: '', storeOrgName: '' }
  ...
  methods: {
      ...
      switch (this.dialogTarget) {
        ...
        case "delegeteBillNo": {
          ...
          if (this.form.data.sourceBillType == '1') {
            ...
            const { data: list } = await api.getHkreceiveRegisterList(pam.billNo);
            ...
            param.client = data.customer
            param.clientName = data.customerName
            ...
            param.supplierName = data.supplierName
            param.client = data.customerCode
            param.clientName = data.customerName
            param.reportGateType = data.reportGateType
        ...
        case "materialCode":
          this.formDialog.ruleForm = {
            ...
            ruleType: data.specificationType,
    ...
    async getItemTableData (searchCode) {
      ...
      try {
        ...
        // }));
        list.forEach((e, i) => {
          e.receiptOrderCode = this.form.data.receiptOrderCode
        ...
        this.table = { ...this.table, tableData: list, isshow: true, pageCount: count };
      } catch (error) {
        console.log(error);
      }
    },
    ...
    async handlePreserve () {
      ...
      this.isLoading = true;
      if (this.status == "add") {
        var { data, status } = await api.changeReceiptOrderData({ receiptBaseList: this.table.tableData, receiptOrder: this.form.data }, "POST");
        this.form.data = { ...this.form.data, status: data.status || '1' }
      } else {
        var data = await api.changeReceiptOrderData(this.form.data, "PUT");
      }
    ...
    handleSubmit () {
      this.utools.submitBusinessStatus({
        code: this.form.data.receiptOrderCode,
        ctx: this,
        form: this.form
      });
    },
    ...
    async handlerTableFormPreservation () {
      ...
      if (this.zbcz == 'add') {
        ...
        if (this.status == 'update') {
          const { data } = await api.changeReceiptBaseData(form);
      ...
      if (this.zbcz == 'update') {
        ...
        if (dataIndex != -1) {
          ...
          if (this.status == 'update') {
            const { data } = await api.changeReceiptBaseData(form, 'PUT');
            data && this.initTableData()
          }
          this.$message.success("操作成功");
        } else {
      ...
      if (this.zbcz == 'delete') {
        ...
        if (dataIndex != -1) {
          ...
          if (this.status == 'update') {
            const { data } = await api.deleteReceiptBaseData(this.clickRowData.itemCode);
            data && this.initTableData()
          }
          this.$message.success("操作成功");
        } else {
    ...
    async initTableData (page = {}) {
      try {
        const {
          data: { list, count }
        } = await api.getReceiptBaseList({
          ...page,
          receiptOrderCode: this.form.data.receiptOrderCode
        });
    ...
    async getCode () {
      try {
        const { data } = await api.getReceiptOrderCode();
        this.form.data.receiptOrderCode = data;
      } catch (error) {
        console.log(error);
      }
    },
