  ...
  <div class="table">
    ...
    <div class="container pd-10">
      ...
      <pagination-component
        ...
        @handlePageChange='queryGodownentry'
      ></pagination-component>
    </div>
  </div>
</template>
...
import tableConfig from '@/domain/tableconfig/logistics/GodownEntry.js'
// 组件
...
export default {
  ...
  computed: {
    ...
    queryBarFormConfig () {
      return [
        { label: "编码", moduleBind: "godownEntryCode", isInput: true },
  ...
  methods: {
    ...
    queryGodownentry (page = {}) {
      appHelper.queryGodownentry(page).then(result => {
    ...
    queryData () {
      this.queryGodownentry(this.queryBar.data)
    },
    goDelete () {
      if (this.currentRow && this.currentRow.godownEntryCode) {
        this.$confirm('此操作将永久删除单据, 是否继续?', '提示', { type: 'warning' }).then(() => {
          appHelper.removeGodownentry(this.currentRow.godownEntryCode).then(data => {
            this.queryGodownentry()
    ...
    jumpForm (type) {
      ...
      this.$router.push(`GodownEntryForm`)
    },
    closeForm () {
      this.popForm = false
    },
