...
import tableconfig from '@/domain/tableconfig/logistics/DistributionTrain'
...
export default {
  ...
  computed: {
    ...
    queryBarFormConfig () {
      return [
        { label: "车次", moduleBind: "deliveryTrain", isInput: true },
        { label: '单据状态', moduleBind: 'status', isSelect: true, selectOption: this.orderStatus },
        { label: "车牌号", moduleBind: "plateNo", isInput: true },
        { label: "司机", moduleBind: "driver", isInput: true },
        { label: "计划出车时间", moduleBind: "outcarTimeFrom", isDate: true },
        { label: "至", moduleBind: "outcarTimeTo", isDate: true },
      ]
    },
    clickTypeAsync () {
      return {
        'search': (data = {}) => api.querydistributiontrainSearch({ ...data, ...this.ruleForm }),//data:数据，ruleForm:分页；
        'add': () => { utools.setSession('DistributionTrain', { jumpType: 'add' }); this.jumpForm('add') },
        'delete': () => { if (this.clickRow.distributionTrainCode !== undefined) return api.deletedistributiontrainData(this.clickRow.distributionTrainCode) },
        'update': () => {          this.titleCallBack(async () => {
            const { data } = await api.searchDistributionData(this.clickRow.distributionTrainCode);
            utools.setSession('DistributionTrain', data)
