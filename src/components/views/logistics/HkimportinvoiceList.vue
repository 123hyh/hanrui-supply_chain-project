...
// 注册表
import tableConfig from "@/domain/tableconfig/logistics/Hkimportinvoice.js";
  ...
  methods: {
    ...
    async initTableList (pageParams = {}) {
      try {
        const {
          data: { list, count }
        } = await api.queryHkimportinvoice({
          ...pageParams,
          ...this.queryBar.data
        });
        (this.pageCount = count),
          (this.table.tableData = list.reduce(
            (pre, cur) =>
              (pre = [
                ...pre,
                {
                  ...cur,
                  sourceBillTypeName:
                    cur.sourceBillType === "1" ? "委托订单" : "卖方交(发)提货"
                }
              ]),
            []
          ));
        ...
        case "delete":
          {
            ...
            try {
              const { status } = await api.removeHkimportinvoice(
                this.clickRow.billNo
              );
              s = status;
              this.initTableList();
            } catch (error) {
