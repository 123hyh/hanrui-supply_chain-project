...
import tableConfig from "@/domain/tableconfig/logistics/Szinstore.js";
...
export default {
  ...
  methods: {
    ...
    querySzinstore (page = {}) {
      appHelper.querySzinstore(page).then(result => {
    ...
    goDelete () {
      if (this.currentRow && this.currentRow.billNo) {
        this.$confirm("此操作将永久删除单据, 是否继续?", "提示", {
          ...
          .then(() => {
            appHelper
              .removeSzinstore(this.currentRow.billNo)
              .then(data => {
                this.querySzinstore();
    ...
    jumpForm (type) {
      ...
      this.$router.push(`szinstoreForm`);
    },
    closeForm () {
      this.popForm = false;
    },
