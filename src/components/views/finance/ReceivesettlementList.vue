...
import tableconfig from '@/domain/tableconfig/finance/Receivesettlement'
...
export default {
  ...
  methods: {
            ...
            this.$alert("删除不可恢复，是否确定删除？", "删除", {
              ...
              beforeClose: (action, instance, done) => {
                if (action === "confirm") {
                  ...
                  setTimeout(() => {
                    // 删除
                    api.deletereceivesettlementData(this.curRowData.billNo).then(res => {
                      if (res) {
    ...
    async queryData () {
      try {
        const { data } = await api.queryreceivesettlementSearch(this.ruleForm);
