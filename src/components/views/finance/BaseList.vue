  ...
  <div class="table">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-menu"></i> {{itemtitle}}
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
...
import tableconfig from '@/domain/tableconfig/finance/ExpenseOrder'
...
export default {
  ...
  methods: {
            ...
            this.$alert("删除不可恢复，是否确定删除？", "删除", {
              ...
              beforeClose: (action, instance, done) => {
                if (action === "confirm") {
                  ...
                  setTimeout(() => {
                    // 删除
                    api.removeexpenseorder(this.curRowData.billNo).then(res => {
                      if (res) {
    ...
    async queryData() {
      try {
        const { data } = await api.queryexpenseorderSearch(this.ruleForm);
