...
// 配置
import tableConfig from '@/domain/tableconfig/finance/Receipt.js'
...
export default {
  ...
  methods: {
    ...
    async queryPage (data = {}) {
      try {
        const { data: { list, count } } = await api.queryReceiptSearch({ ...data, ...this.queryBar.data });
    ...
    async goDelete () {
      if (this.table.activeRow.billNo) {
        ...
        try {
          await api
            .deleteReceiptData(this.table.activeRow.billNo);
          this.queryPage();
        } catch (error) {
          Scode = 0;
          console.log(error)
