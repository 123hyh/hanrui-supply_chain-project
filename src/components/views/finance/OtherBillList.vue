...
import tableconfig from '@/domain/tableconfig/finance/OtherBill'
...
export default {
  ...
  methods: {
            ...
            this.$alert("删除不可恢复，是否确定删除？", "删除", {
              ...
              beforeClose: (action, instance, done) => {
                if (action === "confirm") {
                  ...
                  setTimeout(() => {
                    // 删除
                    api.removeotherbill(this.curRowData.billNo).then(res => {
                      console.log(res)
                      if (res) {
    ...
    async queryData () {
      try {
        const { data } = await api.queryotherbillSearch(this.ruleForm);
