<template>
	<div class="AddCreditBox">
		...
		<!-- 输入框 -->
		<div>
			<el-form :model="accountingForm" ref="accountingForm" label-width="150px" label-position="right" :inline-message="true"
			 class="form-layout">
				<el-form-item v-for="(elem,index) of accountingKey" :key="index" :label="elem.name" :prop="elem.prop">
					<!-- 输入字符串类型 -->
					<el-input v-if="!elem.select && !elem.type" v-model="accountingForm[elem['key']]" style="width: 9vw" :disabled="elem.disabled"
					 :placeholder="accountingForm[elem['key']]" :readonly="elem.readonly">
						<el-button v-if="elem.btn" slot="append" icon="el-icon-search"></el-button>
					</el-input>
					<!--  输入数字类型 -->
					<el-input v-if="!elem.select && elem.type" v-model.number="accountingForm[elem['key']]" style="width: 9vw" :disabled="elem.disabled"
					 :placeholder="accountingForm[elem['key']]" :readonly="elem.readonly">
						<el-button v-if="elem.btn" slot="append" icon="el-icon-search"></el-button>
					</el-input>
					<el-select v-if="elem.select" v-model="accountingForm[elem['key']]" style="width: 9vw" placeholder="请选择选项">
		...
		<div>
			<el-tabs v-model="tabActiveName" type="border-card" @tab-click="handlerTabClick">
				<el-tab-pane label="应收结算单" name="receivable" :disabled='tabIsDisabled'></el-tab-pane>
				<el-tab-pane label="应付结算单" name="payable" disabled='true'></el-tab-pane>
				<el-tab-pane label="收款单" name="voucher" disabled='true'></el-tab-pane>
				<!-- 表格 -->
...
<script>
	import Accounting from "@/domain/entities/finance/Accounting";
	import AccountingForm from "@/domain/formconfig/finance/Accounting";
	...
	export default {
		data() {
			return {
				...
				// 主form
				accountingForm: new Accounting(),
				accountingKey: AccountingForm,
				/*********** 收款单data start***********/
				// table
				dataTab: [],
				// table - 列
		...
		methods: {
			...
			async handlePreserve() {
				const {
					data
				} = await api.addData('/accounting', this.accountingForm)
				if (data) {
					// 提交成功后...
					this.$alert('保存成功', '新增', {
						...
						callback: action => {
							...
							this.accountingForm = new Accounting()
						}
					});
				}
			},
			...
			async handlerTabClick({
				...
				ColumnData = require("@/domain/entities/finance/Accounting" + this.capitalize(name)).default;
				this.columnData = new ColumnData();
				this.columnDataTable = require("@/domain/tableconfig/finance/Accounting" + this.capitalize(name)).default;
				this.columnDataKey = require("@/domain/formconfig/finance/Accounting" + this.capitalize(name)).default;
				const {
					data
				} = await api.querySearch('/accounting' + name, this.queryTableData)
			...
			async goTabPage(index, pageSize) {
				...
				const {
					data
				} = await api.querySearch('/accounting' + this.tabActiveName, this.queryTableData)
				// 替换数据
			...
			// 新增修改
			async saveTab() {
				this.columnData.accountingNo = this.$store.state.accountingNo;
				// this.$refs.columnDataForm.validate().then(() => {
					// 验证成功
					if (this.$store.state.btnType == 'create') {
						const {
							data: addData
						} = await api.addData('/accounting' + this.tabActiveName, this.columnData)
						if (addData) {
							// 提交成功后...
							this.$alert('保存成功', '新增', {
								confirmButtonText: '确定',
						...
						const {
							data: editData
						} = await api.editData('/accounting' + this.tabActiveName, this.columnData)
						if (editData) {
							// 提交成功后...
							this.$alert('修改成功', '修改', {
								confirmButtonText: '确定',
			...
			async openTabForm(btnType) {
				...
				if (btnType == 'update' && itemCode) {
					const {
						data: searchOneData
					} = await api.searchOneData('/accounting' + this.tabActiveName,{itemCode:itemCode})
						...
						beforeClose: (action, instance, done) => {
							if (action === 'confirm') {
								...
								setTimeout(() => {
									// 删除
									console.log(itemCode)
									api.deleteData('/accounting' + self.tabActiveName,
									itemCode).then(res => {
										if (res) {
											// 提交成功后...
											self.$alert('删除成功', '删除', {
