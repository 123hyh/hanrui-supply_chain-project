<template>
	<div class="AddCreditBox">
		...
		<!-- 输入框 -->
		<div>
			<el-form :model="otherbillsForm" ref="otherbillsForm" label-width="150px" label-position="right" :inline-message="true"
			 class="form-layout">
				<el-form-item v-for="(elem,index) of otherbillsKey" :key="index" :label="elem.name" :prop="elem.prop">
					<!-- 输入字符串类型 -->
					<el-input v-if="!elem.select && !elem.type" v-model="otherbillsForm[elem['key']]" style="width: 9vw" :disabled="elem.disabled"
					 :placeholder="otherbillsForm[elem['key']]" :readonly="elem.readonly">
						<el-button v-if="elem.btn" slot="append" icon="el-icon-search"></el-button>
					</el-input>
					<!--  输入数字类型 -->
					<el-input v-if="!elem.select && elem.type" v-model.number="otherbillsForm[elem['key']]" style="width: 9vw" :disabled="elem.disabled"
					 :placeholder="otherbillsForm[elem['key']]" :readonly="elem.readonly">
						<el-button v-if="elem.btn" slot="append" icon="el-icon-search"></el-button>
					</el-input>
					<el-select v-if="elem.select" v-model="otherbillsForm[elem['key']]" style="width: 9vw" placeholder="请选择选项">
		...
		<div>
			<el-tabs v-model="tabActiveName" type="border-card" @tab-click="handlerTabClick">
				<el-tab-pane label="分录" name="entries" :disabled='tabIsDisabled'></el-tab-pane>
				<el-tab-pane label="修改记录" name="modifyrecord" :disabled='tabIsDisabled'></el-tab-pane>
				<!-- 表格 -->
		...
		<!--弹出层： 收款单form -->
		<el-dialog :visible.sync="popFormTab" :close-on-click-modal="false" width="68%" title="收款单">
...
<script>
	import Otherbills from "@/domain/entities/finance/Otherbills";
	import OtherbillsForm from "@/domain/formconfig/finance/Otherbills";
	...
	export default {
		data() {
			return {
				...
				tabActiveName: "entries", // 激活tab菜单
				// 主form
				otherbillsForm: new Otherbills(),
				otherbillsKey: OtherbillsForm,
				/*********** 收款单data start***********/
				// table
				dataTab: [],
				// table - 列
		...
		methods: {
			...
			async handlePreserve() {
				const {
					data
				} = await api.addData('/otherbills', this.otherbillsForm)
				if (data) {
					// 提交成功后...
					this.$alert('保存成功', '新增', {
						...
						callback: action => {
							...
							this.otherbillsForm = new Otherbills()
						}
					});
				}
			},
			...
			async handlerTabClick({
				...
				ColumnData = require("@/domain/entities/finance/Otherbills" + this.capitalize(name)).default;
				this.columnData = new ColumnData();
				this.columnDataTable = require("@/domain/tableconfig/finance/Otherbills" + this.capitalize(name)).default;
				this.columnDataKey = require("@/domain/formconfig/finance/Otherbills" + this.capitalize(name)).default;
				const {
					data
				} = await api.querySearch('/otherbills' + name, this.queryTableData)
			...
			async goTabPage(index, pageSize) {
				...
				const {
					data
				} = await api.querySearch('/otherbills' + this.tabActiveName, this.queryTableData)
				// 替换数据
			...
			// 新增修改
			async saveTab() {
				this.columnData.otherbillsNo = this.$store.state.otherbillsNo;
				// this.$refs.columnDataForm.validate().then(() => {
					// 验证成功
					if (this.$store.state.btnType == 'create') {
						const {
							data: addData
						} = await api.addData('/otherbills' + this.tabActiveName, this.columnData)
						if (addData) {
							// 提交成功后...
							this.$alert('保存成功', '新增', {
								confirmButtonText: '确定',
						...
						const {
							data: editData
						} = await api.editData('/otherbills' + this.tabActiveName, this.columnData)
						if (editData) {
							// 提交成功后...
							this.$alert('修改成功', '修改', {
								confirmButtonText: '确定',
			...
			async openTabForm(btnType) {
				...
				if (btnType == 'update' && itemCode) {
					const {
						data: searchOneData
					} = await api.searchOneData('/otherbills' + this.tabActiveName,{itemCode:itemCode})
						...
						beforeClose: (action, instance, done) => {
							if (action === 'confirm') {
								...
								setTimeout(() => {
									// 删除
									console.log(itemCode)
									api.deleteData('/otherbills' + self.tabActiveName,
									itemCode).then(res => {
										if (res) {
											// 提交成功后...
											self.$alert('删除成功', '删除', {
		...
		created() {
			this.handlerTabClick({name: 'entries'});//初始化tab
		},
	};
</script>
