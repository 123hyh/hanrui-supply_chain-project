	...
	export default {
		...
		methods: {
			...
			handlerSubPreservation(rowData, key) {
				if(this.transferForm[key]!=undefined){
					this.transferForm[key] = rowData[key];
				}
			},
			rousePopover(type, is, currencySelectInputKey) {
				this.searchFormConfig=[];
				if(this.transferForm[type]!=undefined){
					if(type=="originalPaymentAddress"){
						this.searchFormConfig.push({label:'委托方编号',moduleBind:"clientNo",isInput:true});
						this.apiKey = '/client';
						this.itemName = '原付款地址';
						this.popoverListKey = [
							// 设置表格的表头
							...
							{ key: "clientEnName", name: "外文名称" },
						];
					}else if(type=="outEntrust"){
						this.searchFormConfig.push({label:'委托订单编号',moduleBind:"entrustOrderNo",isInput:true});
						this.apiKey = '/entrustorder';
						this.itemName = '转出委托单号';
						this.popoverListKey = [
								// 设置表格的表头
								{ key: "entrustOrderNo", name: "委托订单编号" },
								{ key: "clientName", name: "委托方名称" },
								{ key: "trusteeName", name: "受托方名称" },
								{ key: "status", name: "单据状态" },
							];
					}else if(type=="settlementAdvice"){
						this.searchFormConfig.push({label:'单据编号',moduleBind:"billNo",isInput:true});
						this.apiKey = '/receivesettlement';
						this.itemName = '应收结算单';
						this.popoverListKey = [
								// 设置表格的表头
								{ key: "billNo", name: "单据编号" },
								{ key: "receiveDate", name: "应收日期" },
								{ key: "settleCurrency", name: "结算币别" },
								{ key: "delegeteBillNo", name: "委托单号" },
							];
					}else if(type=="outCustomer"){
						this.searchFormConfig.push({label:'编码',moduleBind:"customerNo",isInput:true});
						this.apiKey = '/customer';
						this.itemName = '转出客户';
						this.popoverListKey = [
								// 设置表格的表头
								{ key: "customerNo", name: "编码" },
								{ key: "customerName", name: "供应链客户名称" },
								{ key: "internalCustomer", name: "是否内部客户" },
								{ key: "customerEnName", name: "外文名称" },
								{ key: "shortCode", name: "助记码" },
							];
					}else if(type=="outSupplier"){
						this.searchFormConfig.push({label:'供应商编码',moduleBind:"supplierCode",isInput:true});
						this.apiKey = '/supplierbase';
						this.itemName = '转出供应商';
						this.popoverListKey = [
								// 设置表格的表头
								...
								{ key: "remenberCode", name: "助记码" },
							];
					}else if(type=="intoEntrust"){
						this.searchFormConfig.push({label:'委托订单编号',moduleBind:"entrustOrderNo",isInput:true});
						this.apiKey = '/entrustorder';
						this.itemName = '转入委托单号';
						this.popoverListKey = [
								// 设置表格的表头
								{ key: "entrustOrderNo", name: "委托订单编号" },
								{ key: "clientName", name: "委托方名称" },
