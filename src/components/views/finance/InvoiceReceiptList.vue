...
import tableconfig from "@/domain/tableconfig/finance/InvoiceReceiptall";
...
export default {
  ...
  computed: {
    objectSpan () {
      ...
      this.tableData.list.forEach(item => {
        for (let i = 0; i < item.invoiceReceiptItemList.length; i++) {
          var row = { ...item.invoiceReceiptItemList[i], ...item };
          if (i == 0) {
            row.rowspan = item.invoiceReceiptItemList.length;
          } else {
            row.rowspan = 0;
          }
          newlist.push(row);
  ...
  methods: {
    ...
    objectSpanMethod ({ row, column, rowIndex, columnIndex }) {
      let uncolumn = ['receiverName', 'invoiceType', 'receivableNo', 'invoiceAmount', 'invoiceNo', 'followGoods', 'remark'] // 行（有跨行或不跨行）中不参与的列
      if (uncolumn.indexOf(column.property) == '-1') {
              ...
              beforeClose: (action, instance, done) => {
                if (action === "confirm") {
                  ...
                  setTimeout(() => {
                    // 删除
                    api
                      .removeinvoicereceipt(this.curRowData.billNo)
                      .then(res => {
                        if (res) {
