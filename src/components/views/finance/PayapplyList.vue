...
export default {
  ...
  data: () => ({
    queryBarFormConfig: [
      ...
      { label: "收款单位", moduleBind: "receiverDepartment", isInput: true }
    ],
    ruleForm: {
      pageIndex: 1,
      pageSize: 10
  ...
  methods: {
            ...
            this.$alert("删除不可恢复，是否确定删除？", "删除", {
              ...
              beforeClose: (action, instance, done) => {
                if (action === "confirm") {
                  ...
                  setTimeout(() => {
                    // 删除
                    api.removepayapply(this.curRowData.billNo).then(res => {
                      if (res) {
    ...
    async queryData () {
      try {
        const { data } = await api.querypayapplySearch(this.ruleForm);
